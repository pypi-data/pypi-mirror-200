...
class RequestWrapper(object):
    ...
    def validate_body(self,
        ...
        if isinstance(validator, Validator):
            ...
            for key, value in validator.items():
                actual_value = self.post_data_json.get(key)
                if actual_value is None:
                    self.add_error('body', key, f'should have value "{value}", but was missing')
                elif actual_value != value:
                    self.add_error('body', key, f'should have value "{value}", not "{actual_value}"')
        else:
...
class EventQueue(object):
    ...
    def registration_filter(self, request):
        ...
        if not re.match(self.url_pattern, request.url):
            return False
        return True

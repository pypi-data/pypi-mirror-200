...
class UserService:
    ...
    def user_list(params=None, allow_user_list=None, ban_user_list=None, need_Pagination=True):
        ...
        # 允许筛选的字段
        user_name = params["user_name"] if params.get("user_name") else None
        email = params["email"] if params.get("email") else None
        phone = params["phone"] if params.get("phone") else None
        full_name = params["full_name"] if params.get("full_name") else None
        nickname = params["nickname"] if params.get("nickname") else None
        allow_user_list = [i for i in allow_user_list if i]
        ...
        try:
            ...
            if user_name:
                user_base_set = user_base_set.filter(user_name__icontains=user_name)
            if email:
                user_base_set = user_base_set.filter(email__icontains=email)
            if phone:
                user_base_set = user_base_set.filter(phone__icontains=phone)
            if full_name:
                user_base_set = user_base_set.filter(full_name__icontains=full_name)
            if nickname:
                user_base_set = user_base_set.filter(nickname__icontains=nickname)

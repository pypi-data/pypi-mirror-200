...
@Component({
  selector: 'jhi-ca-service-accounts',
  templateUrl: './ca-service-accounts.component.html',
})
export class CAServiceAccountsComponent implements OnInit, OnDestroy {
  @ViewChild('cAServiceAccountsTable', { static: true })
  cAServiceAccountsTable!: Table;

  @Output() currentRecord = new EventEmitter<IServiceAccount>();
  ...
  public _currentRecord?: IServiceAccount;
  public selectedRecord?: IServiceAccount;
  ...
  loadPage(page?: number, dontNavigate = true, keepCurrentRecord?: boolean): void {
    ...
    this.serviceAccountService
      ...
      .subscribe(
        (res: HttpResponse<IServiceAccount[]>) => {
  ...
  ngOnInit(): void {
    ...
    this.eventSubscriptions.push(
      this.eventManager.subscribe('CAServiceAccountsRecordUpdated', event => {
        if (typeof event !== 'string') {
          if (event.content) {
            Object.assign(this._currentRecord, event.content as IServiceAccount);
          }
          // after add new record remove the last record
          if (this.serviceAccounts.length > this.itemsPerPage) {
            this.serviceAccounts = this.serviceAccounts.slice(0, this.itemsPerPage);
          }
          if (this.addingNewRecord && this.serviceAccounts[0].id !== undefined) {
            this.addingNewRecord = false;
          }
    ...
    this.eventSubscriptions.push(
      this.eventManager.subscribe('CAServiceAccountsCancelAddNew', () => {
        this.removeNewRecord();
      })
    );

    this.eventSubscriptions.push(
      this.eventManager.subscribe('CAServiceAccountsIsEdited', () => {
        this.isEditing = true;
      })
    );

    this.eventSubscriptions.push(
      this.eventManager.subscribe('CAServiceAccountsCancelEdit', () => {
        this.isEditing = false;
      })
    );
  }
  ...
  onLazyLoadEvent(event: LazyLoadEvent): void {
    ...
    // this.router.navigate(['/ca-service-accounts'], { queryParams });
  ...
  trackId(index: number, item: IServiceAccount): number {
    return item.id!;
  }
  ...
  private getFilterForParentRecord(): string {
    if (this._parentRecord?.id && this._parentRecord.company?.id) {
      return `billingAccount.id==${this._parentRecord.id} + and + company.id==${this._parentRecord.company.id}`;
    }
    return 'id==-9876';
  }

  delete(serviceAccount: IServiceAccount): void {
    this.confirmationService.confirm({
      header: this.translateService.instant('entity.delete.title'),
      message: this.translateService.instant('aportalApp.cAServiceAccounts.delete.question', { id: serviceAccount.id }),
      ...
      accept: () => {
        if (serviceAccount.id) {
          this.serviceAccountService.delete(serviceAccount.id).subscribe(() => {
            this.loadPage();
          });
        } else {
          this.removeNewRecord();
  ...
  protected onSuccess(
    ...
    this.selectFirstRow();
    if (navigate) {
      this.router.navigate(['ca-service-accounts'], {
        queryParams,
      });
    }
  }
  ...
  /*
  selectRow(tableRow: IServiceAccount): void {
...
*/
  ...
  selectFirstRow(): void {
    if (this.serviceAccounts.length > 0) {
      this._currentRecord = this.serviceAccounts[0];
    } else {
  ...
  onRowSelect(): void {
    ...
    this.eventManager.broadcast(new EventWithContent<IServiceAccount | undefined>('CAServiceAccountsRecordChange', this._currentRecord));
  }
  ...
  selectCurrentRecord(): void {
    ...
    if (this.serviceAccounts.length > 0) {
      if (this.selectedRecord?.id) {
        this._currentRecord = this.serviceAccounts.find(item => item.id === this.selectedRecord!.id);
  ...
  selectRow(): void {
    if (this.isEditing) {
      this.confirmationService.confirm({
        ...
        accept: () => {
          this.selectedRecord = this._currentRecord;
          this.eventManager.broadcast('CAServiceAccountsSaveRecord');
        },
        reject: () => {
  ...
  createRecord(): void {
    ...
    const newRecord: IServiceAccount = new ServiceAccount();
    ...
    this.serviceAccounts.unshift(newRecord);
  ...
  removeNewRecord(): void {
    if (this.serviceAccounts.length > 0) {
      if (this.serviceAccounts[0].id === undefined) {
        this.serviceAccounts = this.serviceAccounts.slice(1);
        this.addingNewRecord = false;
        this.selectFirstRow();
      }
    }
  ...
  openDetails(): void {
    if (this._parentRecord) {
      this.globalContextService.setBillingAccountAndRemoveOthers(this._parentRecord);
    }
    if (this._currentRecord?.id && !this.selectedAll) {
      this.router.navigate(['/service-account'], {
        queryParams: { filter: `id==${this._currentRecord.id}` },
      });
    } else {
      this.router.navigate(['/service-account']);
    }
  ...
  openWizNewEnvironmentForm(): void {
    if (this._parentRecord) {
      this.globalContextService.setBillingAccountAndRemoveOthers(this._parentRecord);
    }
    this.router.navigate(['/wiz-new-environment']);
  }
}

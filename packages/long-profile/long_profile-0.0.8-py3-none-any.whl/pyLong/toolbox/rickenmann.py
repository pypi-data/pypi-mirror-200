...
class Rickenmann(zProfile):
    ...
    def duplicate(self):
        """
        duplicate the rickenmann calculation

        returns:
        - new_rickenmann: duplicated rickenmann calculation - Rickenmann

        examples:
        >>> new_rickenmann = rickenmann.duplicate()

        """
        new_rickenmann = Rickenmann()
        new_rickenmann.copy_style(self)

        new_rickenmann.name = f"{self._name} duplicated"
        new_rickenmann.label = self._label

        new_rickenmann.xz = self._xz

        new_rickenmann._results = self._results

        return new_rickenmann
    ...
    def run(self, zprofile, reverse=True, x_start=0., volume=1000., step=1., envelope=False):
        """
        run calculation
        ...
        examples:
        >>> rickenmann.run(zprofile, reserve=True, x_start=1500., volume=10000., step=1., envelope=True)

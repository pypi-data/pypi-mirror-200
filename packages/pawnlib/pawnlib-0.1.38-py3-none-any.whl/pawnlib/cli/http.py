...
import json
import copy
from pawnlib.typing import str2bool, StackList, is_json
...
def get_arguments(parser):
    ...
    parser.add_argument('-i', '--interval', type=float, help='interval sleep time seconds.', default=1)
    parser.add_argument('-m', '--method', type=lambda s : s.upper(), help='method.', default="get")
    parser.add_argument('-t', '--timeout', type=float, help='timeout seconds ', default=10)
    parser.add_argument('-b', '--base-dir', type=str, help=f'base dir for httping', default=".")
    ...
    parser.add_argument('--ignore-ssl', action='count', help='ignore ssl certificate ', default=0)
...
def check_url_process(args):
    ...
    # response_time = int(check_url.response.elapsed)
    response_time = check_url.response.elapsed

    # pawn.console.log(f"{response_time}, {type(response_time)}")
    # pawn.console.log(f"{response_time}")
    args.response_time.push(response_time)
    # pawn.console.log(f"Checking, {check_url.response}")
    ...
    if check_url.is_success():
        ...
        if args.error_stack_count > args.stack_limit:
            ...
            pawn.error_logger.error(f"Error Stack Count: {args.error_stack_count}, SEND_SLACK")
            args.error_stack_count = 0

        # pawn.error_logger.error(f"[red][FAIL] {message}[/red][bold] Error={check_url.response}[/bold]")
        pawn.error_logger.error(f"{message}, Error={check_url.response}")

...
class XGBoostBaseLocalTrainingHandler(LocalTrainingHandler, ABC):
    ...
    def update_model(self, model_update):
        ...
        try:
            ...
            if self.fl_model is None:
                ...
                logger.info(
                    'update_model: Initialized a new FL model object [party_id={}]'.format(self.party_id))
            # Update Model Object
    ...
    def initialize(self, params, party_id=None, last_step='update_raw_preds'):
        ...
        logger.info('initialize: Received and initializing local hyper-parameters [party_id={}]'.format(
            party_id))
        ...
        logger.info(
            'initialize: Performing preliminary checks and initialization.')
    ...
    def init_rejoin(self, party_id, params, curr_round, baseline, model_update, last_step='update_raw_preds'):
        ...
        try:
            ...
            logger.exception('init_rejoin: [party_id={}] encountered an exception and must restart to rejoin [ex={}]'.format(
                party_id, ex))
    ...
    def init_null_preds(self, baseline):
        ...
        logger.info('init_null_preds: Generating Initial Predictions from Null Model [party_id={}]'.format(self.party_id))
    ...
    def update_grad(self):
        ...
        if not hasattr(self, 'n_completed_trains_at_rejoin') or self.n_completed_trains > self.n_completed_trains_at_rejoin:

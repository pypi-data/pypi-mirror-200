...
def parse_speed_duplex(oper_speed: str, oper_duplex: str) -> Tuple[str, str]:
    ...
    """
    auto_speed = ["auto", "100 Gb/s", "10 Gb/s"]
    if oper_speed in auto_speed:
        speed = "auto"
    elif "1000 Mb/s" in oper_speed:
        speed = "1000"
    else:
        speed = "auto"

    if "half" in oper_duplex:
        duplex = "half"
    elif "full" in oper_duplex:
        duplex = "full"
    else:
        duplex = "auto"
...
class NXOS(BaseDevice):
    ...
    def get_interface_details(self, interface: Optional[str] = None) -> List[Interface]:
        ...
        for entry in parsed:
            ...
            i_name = entry["interface"].replace("Ethernet", "Eth")
            i_name = i_name.replace("port-channel","Po")
            interface = Interface(
                name=i_name,

        ...
        **kwargs,
        ...
        if send_access_token and self._auth_provider:
            if self._auth_provider.access_token:
                get_params.update({"access_token": self._auth_provider.access_token})

        if send_project_auth and not send_access_token and self._auth_provider:
            if self._auth_provider.project_id:
                get_params.update({"project_id": self._auth_provider.project_id})
            if self._auth_provider.server_secret:
                get_params.update({"server_secret": self._auth_provider.server_secret})
            if (
                self._auth_provider.client_secret
                and not self._auth_provider.server_secret
            ):
                get_params.update({"client_secret": self._auth_provider.client_secret})
        ...
        http_response = (
            requests.post(request_kwargs)
            if body_payload
            else requests.get(request_kwargs)
        )

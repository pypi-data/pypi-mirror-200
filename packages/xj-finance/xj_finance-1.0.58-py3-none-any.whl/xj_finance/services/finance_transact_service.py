...
class FinanceTransactService:
    ...
    def finance_flow_writing(params, finance_type=None):
        ...
        summary_content = "【" + user_set[
            'full_name'] + "】" + action + "【" + platform_user_name + "】项目名称【" + project_name + "】款项"
        ...
        if finance_type == "RECHARGE" or finance_type == "TRANSFER":
            try:
                ...
                user_finance_data['money_flow'] = "RECHARGE"  # 资金流行为
                ...
                platform_finance_data['money_flow'] = "RECHARGE"  # 沙盒应收
    ...
    def post(param):
        ...
        account_wechat_appid = param.get('account_wechat_appid', '')
        if account_id:
            ...
            item['account'] = BaseInfo.objects.filter(wechat_appid=account_wechat_appid).first()
            if not item['account']:
                return None, '用户account_id不存在'
        ...
        if param.get('money_flow', "") == "RECHARGE" and transact_no[transact_no.rfind('-'):] == "-1":
        ...
        elif param.get('their_wechat_appid', ''):
            their_wechat_appid = param.get('their_wechat_appid', '')
        ...
        if not param.get('their_account_id', '') and their_account_name:
            ...
            base_info = BaseInfo.objects.filter(wechat_appid=their_wechat_appid).first()
            if base_info:
                base_info = model_to_dict(base_info)
                item['their_account_id'] = base_info['id']
                item['their_account'] = BaseInfo.objects.get(id=item['their_account_id'])
                ...
                new_user_param = {
                    "wechat_appid": their_wechat_appid,

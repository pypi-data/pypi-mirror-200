...
class DashboardSession:
    ...
    Args:
        ...
        data_source (str, optional): The location where project data is stored.
            Default is current year Sharepoint data. To pull S3 data add 's3' to the variable.
    ...
    def get_project(self, project_name:str|Project, get_neighbors:bool=True, report_type:str = 'monthly') -> Project:
        """
        ...
        if isinstance(project_name, Project):
            ...
            try:
                ...
                self.add_project(project_name, get_neighbors=get_neighbors, report_type=report_type)
                project = self.project_list[project_name]
 
        return project


    def add_project(self, project_name:str|Project|list, get_neighbors:bool = True, report_type:str = 'monthly') -> Project:
        ...
        if isinstance(project_name, list):
            ...
            project = self.add_projects(project_list, get_neighbors=get_neighbors, report_type=report_type)
            ...
            project = self.__initialize_project(project_name, get_neighbors=get_neighbors, report_type=report_type)
    ...
    def add_projects(self, project_list:list|str|Project, get_neighbors:bool = True, report_type:str = 'monthly') -> Project:
        ...
        if isinstance(project_list, (str, Project)):
            project = self.add_project(project_list, get_neighbors=get_neighbors, report_type=report_type)
            return project
        elif isinstance(project_list, list):
            successful_projects = []
            for project_name in project_list:
                try:
                    project = self.add_project(project_name, get_neighbors=get_neighbors, report_type=report_type)
    ...
    def __initialize_project(self, project_name:str, get_neighbors:bool = True, report_type:str = 'monthly'):
        """Initializes a project object with metadata

        Args:
            project_name (str): Name of a project to initialize.
            get_neighbors (bool): Flag to add & initialize neighbor projects.
            report_type (str): Method for processing the project. Options are `monthly`, `ul`, or `weekly`.
        ...
        try:
            ...
            error_source = traceback.tb_next.tb_frame.f_code # type: ignore
            lineno = traceback.tb_next.tb_lineno # type: ignore
            file = error_source.co_filename
            func = error_source.co_name
            
            print(f"Error initializing {project_name}.")
            print(f"Error details: {error_type} on line {lineno} of `{func}` in {file}")
            proj_init_dict['error_info'] = f"{error_type} on line {lineno} of `{func}` in {file}"
            project = Project(proj_init_dict)
    ...
    def process_project(self, project_name:str|Project|list, reprocess:bool = False, datasub:bool = False,
    ...
    def export_projects(self, project_list:list|str, server:str = 'dev', dest:str|list = 'all', report_type:str = 'monthly',
                        method:str = 'series', use_solcast:bool = True, save_pickle:bool = True, **kwargs) -> None:

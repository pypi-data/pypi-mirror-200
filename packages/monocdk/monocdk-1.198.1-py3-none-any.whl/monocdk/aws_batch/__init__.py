    ...
    class ComputeResourcesProperty:
        def __init__(
            ...
            *,
            ...
            :param desiredv_cpus: The desired number of Amazon EC2 vCPUS in the compute environment. AWS Batch modifies this value between the minimum and maximum values based on job queue demand. .. epigraph:: This parameter isn't applicable to jobs that are running on Fargate resources. Don't specify it. > AWS Batch doesn't support changing the desired number of vCPUs of an existing compute environment. Don't specify this parameter for compute environments using Amazon EKS clusters. > When you update the ``desiredvCpus`` setting, the value must be between the ``minvCpus`` and ``maxvCpus`` values. Additionally, the updated ``desiredvCpus`` value must be greater than or equal to the current ``desiredvCpus`` value. For more information, see `test <https://docs.aws.amazon.com/batch/latest/userguide/error-desired-vcpus-update.html>`_ in the *AWS Batch User Guide* .
        ...
        def desiredv_cpus(self) -> typing.Optional[jsii.Number]:
            ...
            .. epigraph::
               ...
               Additionally, the updated ``desiredvCpus`` value must be greater than or equal to the current ``desiredvCpus`` value. For more information, see `test <https://docs.aws.amazon.com/batch/latest/userguide/error-desired-vcpus-update.html>`_ in the *AWS Batch User Guide* .

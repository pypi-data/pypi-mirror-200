...
class InputConfig:
    def __init__(self, context: CertoraContext) -> None:
        ...
        self.solc_allow_path = context.solc_allow_path if is_new_api() else context.path
...
class CertoraBuildGenerator:
    ...
    def collect_for_file(self,
        ...
        if not is_vyper:
            ...
            main_path = route_func(self.input_config.solc_allow_path)
            ...
            remappings = []
            collect_cmd = f'{solc_ver_to_run} -p "{self.input_config.solc_allow_path}" -o "{compilation_path}" ' \
                          f'--standard-json'
    ...
    def resolve_slot(self, primary_contract: str, slot_name: str) -> str:
        ...
        asm_collect_cmd = f'{solc_ver_to_run} {solc_add_extra_args} -o {self.config_path}/ --overwrite --asm ' \
                          f'--allow-paths "{self.input_config.solc_allow_path}" "{abs_posix_path(file)}"'
        if self.input_config.packages is not None:

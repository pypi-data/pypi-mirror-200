import time, json, logging, threading, re, io, base64
...
class WeixinClient:
    ...
    def send_image(self, wxid: str, data: str):
        """
        发送图片消息
        :param wxid: 发送用户
        :param data: 图片base64
        ...
        """
        self._check_login()
        path = self.helper.Base64ToFile("", data)
        logger.debug("send_image wxid:%s, path:%s", wxid, path)
        self.helper.SendImage(wxid, path)

    def send_file(self, wxid: str, data: str, name: str = ""):
        """
        发送文件消息
        :param wxid: 发送用户
        :param data: 文件base64
        :param name: 文件名称，默认随机文件名
        :return:
        """
        self._check_login()
        path = self.helper.Base64ToFile(name, data)

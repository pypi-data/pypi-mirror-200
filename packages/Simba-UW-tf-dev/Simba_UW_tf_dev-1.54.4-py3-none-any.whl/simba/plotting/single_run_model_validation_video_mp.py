...
def _multiprocess_validation_video(data: pd.DataFrame,
    ...
    while current_frm < end_frm:
        ...
        if gantt_setting == 'Gantt chart: final frame only (slightly faster)':
            ...
            gantt_img = create_gantt_img(bouts_df, clf_name, current_frm, video_meta_data['fps'], 'Behavior gantt chart')
            gantt_img = imutils.resize(gantt_img, height=video_meta_data['video_height'])
...
class ValidateModelOneVideo(ConfigReader):
    ...
    def __create_video(self):
        ...
        with multiprocessing.Pool(self.cores, maxtasksperchild=self.maxtasksperchild) as pool:
            ...
            for cnt, result in tqdm(enumerate(pool.imap(constants, data, chunksize=self.multiprocess_chunksize))):

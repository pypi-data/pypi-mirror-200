...
class FishFeatureExtractor():
    def __init__(self, config_path: str):
        ...
        self.roll_windows_values = check_minimum_roll_windows([10, 4, 2, 1, 0.1, 0.05], self.video_info_df['fps'].min())
        self.roll_windows_values = [x for x in self.roll_windows_values if x >= 1]
    ...
    def calc_border_distances(self):
        ...
        for side in ['left', 'right', 'top', 'bottom']:
            self.csv_df_combined[f'Mean_bp_distance_to_{side}_border'] = self.csv_df_combined[[c for c in self.csv_df_combined.columns if f'distance_to_{side}_border' in c]].mean()
            for window in self.roll_windows_values:

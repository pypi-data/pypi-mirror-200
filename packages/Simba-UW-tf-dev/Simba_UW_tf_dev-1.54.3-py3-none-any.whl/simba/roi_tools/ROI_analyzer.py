...
class ROIAnalyzer(ConfigReader):
    ...
    def __transpose_dicts_to_dfs(self):
        ...
        self.detailed_df = pd.DataFrame(columns=['VIDEO', 'ANIMAL', 'SHAPE', 'ENTRY FRAMES', 'EXIT FRAMES'])
        for video_name, video_data in self.entries_exit_dict.items():
            for animal_name, animal_data in video_data.items():
                for shape_name, shape_data in animal_data.items():
                    df = pd.DataFrame.from_dict(shape_data).rename(
                        columns={'Entry_times': 'ENTRY FRAMES', 'Exit_times': 'EXIT FRAMES'})
                    df['VIDEO'] = video_name
        ...
        self.detailed_df = self.detailed_df[['VIDEO', 'ANIMAL', 'SHAPE', 'ENTRY FRAMES', 'EXIT FRAMES']]

...
from broker_grpc_lib.lib.broker_handler import BrokerHandler, RequestType
from broker_grpc_lib.lib.grpc_client import GRPCClient
from broker_grpc_lib.exc.no_queue_exception import NoReadQueueException, NoWriteQueueException
...
class RabbitMQHandler(BrokerHandler):
    ...
    def __init__(
            ...
            schema: Union[Type[T], Type[BaseModel]],
            max_connections: int,
            max_channels: int,
        ...
        self._loop.run_until_complete(self.__init_pools())
        self._loop.run_until_complete(self.__init_queues())

    async def __init_pools(self):

...
{
    static const char *kwlist[] = { "table", "source", "header", "delimiter", 0 };

    PyObject* table;
    PyObject* source;
    int header = 0;
    char* szDelimiter = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "UO|pz", (char**)kwlist, &table, &source, &header, &szDelimiter))
        return 0;
    ...
    PyObject* sql = PyUnicode_FromFormat("copy %U from stdin with csv %s delimiter '%s'", table, header_token, pszDelimiter);

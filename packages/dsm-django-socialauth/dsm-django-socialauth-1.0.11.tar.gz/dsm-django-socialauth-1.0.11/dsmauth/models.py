...
def logged_in_handle(sender, user, request, **kwargs):
    ...
    if prov.exists():
        ...
        try:
            ...
            user_data.update({
                'image': ImageFile(BytesIO(requests.get(api.get('profile_url')).content), name=f"{user.username}") if api.get('profile_url', "") != "" else None,
                'role': api.get('role', []),
                'permission': api.get('permission', [])
            })
            if not account.exists(): Account.objects.create(**user_data)

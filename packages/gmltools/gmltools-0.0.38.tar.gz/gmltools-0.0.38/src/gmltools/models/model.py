...
def train_bayes_or_grid_search(X_train,y_train,bayes_pbounds,grid_params,random_params,n_jobs,pipe,scoring,bayes_int_params,bayes_n_iter,sample_weight=None,cv=10,random_n_iter=10):
    ...
    if sample_weight is not None : #Sample weight are for XGBoost ando some other models but not fo all
        ...
        if pipe.steps[-1][0]=='XGB':
            ...
            model_.fit(X_train, y_train, LGBM__sample_weight=sample_weights)
        elif pipe.steps[-1][0]=='RF':
            model_.fit(X_train, y_train, RF__sample_weight=sample_weights)
        elif pipe.steps[-1][0] == 'KNN':
            model_.fit(X_train, y_train, KNN__sample_weight=sample_weights)
        elif pipe.steps[-1][0] == 'LR':
            model_.fit(X_train, y_train, LR__sample_weight=sample_weights)
        elif pipe.steps[-1][0] == 'MLP':
            model_.fit(X_train, y_train, MLP__sample_weight=sample_weights)
        elif pipe.steps[-1][0] == 'DT':
            model_.fit(X_train, y_train, DT__sample_weight=sample_weights)
    else :

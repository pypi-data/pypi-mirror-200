...
def _process_dataset_date_logic(
    ...
    n_levels = len(records[0]["bids"])
    price_array = np.empty((n_records, 2, n_levels))
    quantity_array = np.empty((n_records, 2, n_levels))
...
def process(
    ...
    process_dataset(basket, dates, storage, n_jobs)

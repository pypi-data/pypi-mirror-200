...
async def set_fsub_cmd(c, m):
    ...
    try:
        ID = m.command[1]
        LINK = await c.create_chat_invite_link(int(ID), creates_join_request=True)
        await save_group_settings(grp_id, 'fsub', ID)
        await m.reply_text(f"Your Channel is Added\n\nNow You Can See Your Channel Link\n\nYour Channel Link : {LINK.invite_link}", quote=True)

    except Exception as e:
        await save_group_settings(grp_id, 'fsub', False)
        await m.reply_text(
            f"Error occoured!! {e}\n\nAuto Removed Your Fsub Channel\n\nAlso Make Me Admin in Your Channel/Group\n\nYou Can Also Contact Our Support Group For Solve This issue\n\nFormat - `/set_fsub -100xxxxxxxxx`",
            quote=True,
        )

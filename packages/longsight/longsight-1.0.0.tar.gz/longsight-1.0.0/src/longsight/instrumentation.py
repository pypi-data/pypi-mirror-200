...
from aws_lambda_powertools.logging.logger import PowertoolsFormatter
...
def instrument(bucket: str = "", create_aws=False, cloudwatch_push=False):
    """
...
class Instrumentation(AbstractContextManager):
    ...
    def __init__(
        ...
        if cloudwatch_push:
            ...
            self._logger = Logger(logger_handler=handler)
            """
            handler = watchtower.CloudWatchLogHandler(
                boto3_client=self._aws_connector.log_client
                if self._aws_connector

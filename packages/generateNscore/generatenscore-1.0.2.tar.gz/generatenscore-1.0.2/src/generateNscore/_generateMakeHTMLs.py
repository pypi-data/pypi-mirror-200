...
##    if 'figure(' in txt:
##        figureKey = re.findall(figurePattern, txt)[0].strip()
##        if not figureKey or figureKey not in figures:
##            txt=txt.replace(f'figure({item})',f'figure({item}):Error of not finding it')
##        # 할 일
##        # "<div id="canvas2nd"></div>" 추가
##        # 함수 끝에 graph(); 추가
##        # 새로운 함수 function graph() {... } 추가
...
##            more=[]
##            for item in xtra[1]:
##                indx=xtra[0]['choices'].index(item)
##                more.append(xtra[0]['cols'][indx])
            more = [xtra[0]['cols'][xtra[0]['choices'].index(item)] for item in xtra[1]]

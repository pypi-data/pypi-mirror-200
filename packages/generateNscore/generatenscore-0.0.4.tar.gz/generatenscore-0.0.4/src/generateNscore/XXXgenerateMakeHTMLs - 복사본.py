...
def mkHTMLs(work, figures=None):
    ...
    for std, v in work.Sheets.items():
        ...
        if list4HTML: # 단순한 설명 html 내용이 있는 경우
            ...
            for j, item in enumerate(qna):
                if j in list4HTMLindices: # 단순한 설명 html인 경우
                    ...
                    blockORnone='document.getElementById("Qs").style.display = "none";'
                    for indx in list4HTMLindices:
                        if indx == j:
                            blockORnone +=f'document.getElementById("HTML{indx}").style.display = "block";'
                        else:
                            blockORnone +=f'document.getElementById("HTML{indx}").style.display = "none";'
                    func = func.replace('BLOCKorNONE', blockORnone).replace('JS_CODE', 'cnvs.height=0;')
                else: # 질문인 경우
                    tmp=mkHtmlFuncDisplayQ(j, item[0], None, figures)
                    if tmp and not tmp.startswith('Error'):
                        func += tmp
                    else:
                        #print(f'질문지 생성 에서 오류 발생....{tmp}')
                        return f'Error on generating Sheets....{tmp}'
                    
                    blockORnone='document.getElementById("Qs").style.display = "block";'
                    for indx in list4HTMLindices:
                        blockORnone +=f'document.getElementById("HTML{indx}").style.display = "none";'
                    func = func.replace('BLOCKorNONE', blockORnone)
        else: # 예전과 동일
            func='<script type="application/javascript">\n'
            for j, item in enumerate(qna):
                tmp=mkHtmlFuncDisplayQ(j, item[0], None, figures)
        ...
        for jj in range(len(qna)):
            #file +=f'<input type="button" onclick="javascript:display_Q{jj}();" style="font-family: Palatino Linotype;  font-size: 18px" value=" Q-{jj+1} " /> &emsp;\n'
            #file += f'<input type="button" onclick="javascript:display_Q{jj}();" class= "btn" value=" Q-{jj+1} " /> &emsp;\n'
        ...
        if work.Flag4Answer:
            if work.Flag4Sample:
                ...
                #file += JS4TABLE.replace('FILENAME', f'{work.Name}{std}')
...
def mkHtmlFuncDisplayQ(qJ, txt, correctAnswer=None, figures=None):
    ...
    out=templateF.replace('TEXT', f'<font color=red><strong>{qJ+1}.</strong></font> '+txt).replace('qNUM', f'{qJ}')
...
templateBodyR='''<hr> <div id="Qs" style="word-break:keep-all; padding: 5px 20px 10px 20px; background-color: #eee; border: 1px solid #999; display: block; font-family: malgun;  font-size: 18px;"></div>
...
JS4TABLE='''<script type="text/javascript">

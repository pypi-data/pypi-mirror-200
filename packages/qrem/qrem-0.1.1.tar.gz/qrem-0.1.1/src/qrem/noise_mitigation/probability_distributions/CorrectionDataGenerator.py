...
class CorrectionDataGenerator(NoiseModelGenerator):
    ...
    def _construct_local_full_noise_model(self,
        ...
        if intersection_i is None:
            intersection_i = qrem_utils.lists_intersection(neighbors_i, cluster_j)
        if intersection_j is None:
            intersection_j = qrem_utils.lists_intersection(neighbors_j, cluster_i)
    ...
    def _sort_qubits_matrix(
        ...
        # This is our target
        sorted_qubits_dictionary = anf.enumerate_dict(sorted(all_qubits_list))
        # This is what we have
        qubits_dictionary = anf.enumerate_dict(all_qubits_list)
    ...
    def _compute_pair_noise_matrix(self,
        ...
        if cluster_i == cluster_j:
            ...
            averaged_matrix_clusters_i_j = self.get_noise_matrix_averaged(
                sorted(qrem_utils.lists_sum(cluster_i, cluster_j)))
    ...
    def _compute_pair_correction_data(self,
        ...
        whole_marginal = sorted(qrem_utils.lists_sum(cluster_i, cluster_j))
    ...
    def get_pairs_correction_data(self,
        ...
        for qubit in range(self._number_of_qubits):
            ...
            cluter_now = self._clusters_labels_dictionary[(qubit,)]
            self._correction_indices[(qubit,)] = cluter_now

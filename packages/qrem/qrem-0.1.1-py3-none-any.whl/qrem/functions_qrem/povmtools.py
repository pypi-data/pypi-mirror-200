...
def spectral_decomposition(matrix: np.ndarray, hermitian=False):
    ...
    projectors = [qrem_math.outer_product(
...
def get_su2_parametrizing_angles(m_a):
    ...
    if qrem_math.is_pauli_x(m_a):
        return ['x']
    elif qrem_math.identity_check(m_a):
        return ['id']

    m_a = qrem_math.round_array_to_ndigits(m_a)
...
def sort_things(stuff_to_sort: Iterable, according_to_what: Iterable) -> Iterable:
    ...
    return [element for _, element in sorted(zip(X, Y), key=lambda pair: pair[0])]

...
def __format_counts_from_clusters(counts_dict,
    ...
    for sample_list, number_of_times in counts_dict.items():
        ...
        for cluster_index in range(number_of_clusters):
            ...
            local_bitstring_now = anf.integer_to_bitstring(
...
def sample_from_column_of_stochastic_map(column_bitstring,
    ...
    for integer in range(len(samples)):
        counts_dictionary[anf.integer_to_bitstring(integer=integer,
        ...
        if old_sampling:
            ...
            for global_output_state, number_of_counts in counts_dict.items():
                ...
                for cluster_index in range(number_of_clusters):
                    ...
                    cluster_state_bitstring = anf.integer_to_bitstring(global_output_state[cluster_index],
                                                                       len(cluster_now))
...
def sample_from_multiple_input_states_alternative(input_states_dictionary: Dict[str, int],
    ...
    local_registers_dictionary = {len(cluster):
                                      [tuple(anf.integer_to_bitstring(index_outcome, len(cluster)))
...
def sample_from_multiple_input_states_alternative_batches(input_states_dictionary: Dict[str, int],
    ...
    if multiprocessing:
        ...
        res_mp = anf.wrapped_multiprocessing_function(tuple_of_main_arguments=main_tuples,
    ...
    if multiprocessing:
        ...
        res_mp = anf.wrapped_multiprocessing_function(tuple_of_main_arguments=batches_mp,
    ...
    if new_sampling:
        ...
        for cluster in clusters_list:
            ...
            for index_outcome in range(len(local_output_states)):
                local_samples += [anf.integer_to_bitstring(index_outcome, len(cluster))] * \
                                 local_output_states[index_outcome]

            #         list(np.full(shape=local_output_states[index_outcome],
            #                                   fill_value=anf.integer_to_bitstring(index_outcome,
            #                                                                       len(cluster))
            #                                   ))
            # # t3 = time.time()
...
#                                                                          fill_value=anf.integer_to_bitstring(
#                                                                              integer=index,
#                                                                              number_of_bits=len(cluster))))

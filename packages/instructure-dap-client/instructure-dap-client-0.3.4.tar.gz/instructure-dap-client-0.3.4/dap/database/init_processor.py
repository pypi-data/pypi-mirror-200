...
class InitProcessor(BaseInitProcessor):
    ...
    async def process(self, record: JsonRecord, obj: ContextAwareObject) -> None:
        copy_record = self._convert_record_for_copy(record)
        await self._db_operations.copy(copy_record, obj)

    def _convert_record_for_copy(self, record: JsonRecord) -> Tuple:
        return tuple(converter(record) for converter in self._converters)

...
def __initialize_app(base_path, mapped_folder):
  ...
  config_dir = dotenv.get_key(os.path.join(code_base_path, ".env"), "CONFIGS")
  if config_dir is None:
    config_dir = "dashboard"

  if str(config_dir).startswith("/") or str(config_dir).startswith("\\"):
    config_dir = config_dir[1:]
    
  config_dir_full_path = os.path.join(mapped_folder, config_dir)
  os.system(f"mkdir -p {config_dir_full_path}")
  with open(os.path.join(config_dir_full_path, "fhir.json"), "w") as file:
    file.write(FILES.FHIR_CONFIG)

...
def ValidString():
    def f(str_data):
        __string_valid(str_data, msg=None)
        return str_data
    return f

def ValidInt():
    def f(int_data):
        __int_valid(int_data, msg=None)
        return int_data
    return f

def ValidAmount():
    def f(float_data):
        __amount_valid(float_data, msg=None)
        return float_data
    return f

def ValidBoolean():
    def f(bool_data):
        __boolean_valid(bool_data, msg=None)
...
def ValidDate():
    ...
    def f(date_text):
        __date_valid(date_text, msg=None)
        return date_text
    return f
...
def ValidDateTime():
    ...
    def f(datetime_text):
        __datetime_valid(datetime_text, msg=None)
        return datetime_text
    return f

def ValidTime(msg=None):
    def f(time_text):
        __time_valid(time_text, msg=None)
        return time_text
    return f
...
def AllowedPassword():
    ...
    ValidString()
    def f(password):
        __password_valid(password, msg=None)
        return str(password)
    return f

def OnlyDigits(msg=None):
    def f(text):
        __string_of_integers_valid(text, msg=None)
        return text
    return f
...
def __date_valid(str_data, msg=None):
    __string_valid(str_data, msg)
    try:
        datetime.strptime(str_data, date_format)
    except ValueError:
        raise Invalid(msg or 'Date should be of the format {}'.format(date_pattern))

...
def download(uuid, media_type, post_text, media_resource_url, audio_resource_url):
    ...
    url = utils.upload(savePath, is_test(), curGroupId)
    if url == None:
    ...
    if os.path.exists(coverPath) and media_type == "video":
        ...
        coverossurl = utils.upload(coverPath, is_test(), curGroupId)

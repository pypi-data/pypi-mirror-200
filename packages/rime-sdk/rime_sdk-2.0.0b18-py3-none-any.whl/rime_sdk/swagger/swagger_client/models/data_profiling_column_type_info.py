...
class DataProfilingColumnTypeInfo(object):
    ...
    def min_nunique_for_numeric(self):
        ...
        Specifies a minimum number of unique values in a column for that column  to count as a numeric column. Columns that do not meet this minimum are considered as categorical columns.  # noqa: E501
    ...
    def min_unique_prop(self):
        ...
        When the data has a number of unique values equal to at least the value of the min_unique_prop variable, classify the column as a column that  requires unique values.  # noqa: E501
    ...
    def numeric_range_inference_threshold(self):
        ...
        The percent of non-null values that must fall within an inferrable numeric range ([0,1], [0,inf), (-inf, inf)) in order to be inferred as the valid range for a numeric column. By default, this value is 1.0,  requiring all non-null values to fall within the range for that range to be inferred. For example: if 98% of feature X falls in [0, 1] but 100% of feature X falls in [0, inf) then we'll infer [0, inf) as the valid range for feature X. However, if this threshold is 0.98, we'll instead infer [0, 1] as feature X's range.  # noqa: E501

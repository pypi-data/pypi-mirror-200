...
class MyServer(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path in ["/", "/index.html"]:
        ...
        elif self.path.find(".html") != -1 or self.path.find(".css") != -1:
            logger.debug("requesting %s/%s", self.home, self.path)

            try:
                f = open(os.path.join(self.home, os.path.basename(self.path)), 'r', encoding="utf-8")
            ...
            logger.debug("requesting %s", self.path)

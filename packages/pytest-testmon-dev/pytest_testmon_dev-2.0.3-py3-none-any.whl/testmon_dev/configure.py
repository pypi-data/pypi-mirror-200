...
def _get_notestmon_reasons(options):
    if options["no-testmon-dev"]:
        return "deactivated through --no-testmon-dev"

    if options["testmon-dev_noselect"] and options["testmon-dev_nocollect"]:
    ...
    if not any(
        ...
        for t in [
            "testmon-dev",
            "testmon-dev_noselect",
            "testmon-dev_nocollect",
            "testmon-dev_forceselect",
            "tmnet-dev",
        ]
    ):
...
def _get_nocollect_reasons(
    ...
    if options["testmon-dev_nocollect"]:
        return [None]
...
def _get_noselect_reasons(options):
    if options["testmon-dev_forceselect"]:
        return []

    if options["testmon-dev_noselect"]:
        return [None]
...
def _header_collect_select(
    ...
    if notestmon_reasons:
        return TmConf("testmon-dev: " + notestmon_reasons, False, False)

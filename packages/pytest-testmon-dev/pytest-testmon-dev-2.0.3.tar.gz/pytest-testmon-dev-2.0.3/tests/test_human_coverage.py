...
from tests.coveragepy.coveragetest import CoverageTest
from tests.test_process_code import code_samples
...
class TestmonCoverageTest(CoverageTest):
    ...
    def check_human_coverage(self, text, lines=None, fingerprints=None):
        ...
        assert hc == lines
        if fingerprints:
            assert create_fingerprint(m, lines) == fingerprints
...
class DeactivatedBasicTestmonCoverageTest(TestmonCoverageTest):
...
class DeactivatedNewTestmonCoverageTest:
    def parse(self, text):

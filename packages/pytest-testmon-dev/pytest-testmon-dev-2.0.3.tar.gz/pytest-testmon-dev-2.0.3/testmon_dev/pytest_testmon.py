...
def pytest_addoption(parser):
    group = parser.getgroup(
        "automatically select tests affected by changes (pytest-testmon-dev)"
    )
    ...
    group.addoption(
        "--testmon-dev-noselect",
        action="store_true",
        dest="testmon-dev_noselect",
        help=(
    ...
    group.addoption(
        "--testmon-dev-forceselect",
        action="store_true",
        dest="testmon-dev_forceselect",
        help=(
    ...
    # #ifdef SUBPROCESS
    group.addoption(
        "--testmon-dev-singleprocess",
        action="store_false",
        dest="testmon-dev_singleprocess",
    ...
    parser.addini("testmon_dev_url", "URL of the testmon.net api server.")
    parser.addini("testmon_dev_api_key", "testmon-dev api key")
...
def testmon_options(config):
    ...
    for label in [
        ...
        # #ifdef SUBPROCESS
        "testmon-dev_singleprocess",
        # #endif
    ]:
        if config.getoption(label):
            result.append(label.replace("testmon-dev_", ""))
    return result
...
def init_testmon_data(config: Config):
    ...
    packages = ", ".join(
        sorted(
            str(p) for p in pkg_resources.working_set  # pylint: disable=not-an-iterable
        )
    )

    url = config.getini("testmon_dev_url")
    api_key = config.getini("testmon_dev_api_key")
    rpc_proxy = None

    if config.testmon_dev_config.tmnet or getattr(config, "tmnet", None):
...
def pytest_configure(config):
    ...
    config.testmon_dev_config: TmConf = tm_conf
    if tm_conf.select or tm_conf.collect:
        # #ifdef PYTEST-TMNET
        if config.pluginmanager.has_plugin(
            "TestmonSelect"
        ) or config.pluginmanager.has_plugin("TestmonCollect"):
            pytest.exit("You cannot use two instances of testmon plugin.")
        # #endif
...
def pytest_report_header(config):
    tm_conf = config.testmon_dev_config
    ...
    def pytest_sessionfinish(self, session):  # pylint: disable=unused-argument
        if self._host in ("single", "controller"):
            self.testmon_data.db.finish_execution(
                ...
                session.config.testmon_dev_config.select,
            )
        self.testmon.close()
...
class TestmonSelect:
    ...
    def pytest_ignore_collect(self, path, config):
        ...
        if strpath in self.deselected_files and self.config.testmon_dev_config.select:
            return True
        return None
    ...
    def pytest_collection_modifyitems(
        ...
        if self.config.testmon_dev_config.select:
            items[:] = selected
    ...
    def pytest_terminal_summary(self):
        ...
        (
            ...
            total_saved_tests,
            total_tests_all,
        ) = self.testmon_data.fetch_saving_stats(self.config.testmon_dev_config.select)

        terminal_reporter = TerminalReporter(self.config)
        potential_or_not = ""
        if not self.config.testmon_dev_config.select:

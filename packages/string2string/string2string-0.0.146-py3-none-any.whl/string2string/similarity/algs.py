...
class JaroSimilarity:
    ...
    def compute(self,
        ...
        Definition:
            Let m denote the number of matching characters.
                s_i and t_j are assumed matching if s_i == t_j and |i - j| < k,
                    where k = floor((max(len(s), len(t)) / 2) - 1).
            Let t denote the number of transpositions.
                If m > 0, t is half the number of matching characters that are not in the right order in the other string.

            Then, the Jaro similarity is defined as:
                Jaro (S, T) =
                    {
                        Case 1: 0 if m = 0
                        Case 2: 1/3 * (m/|S| + m/|T| + (m - t)/m) otherwise
                    }

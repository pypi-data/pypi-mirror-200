...
class Scene(MiniWorldEnv):
    ...
    def _gen_world(self):
        ...
        self.redFlag = self.place_entity(MeshEnt(mesh_name=f'flagRed', height=2, static=False), pos=start_info['RedFlagPos'])
        self.blueFlag = self.place_entity(MeshEnt(mesh_name=f'flagBlue', height=2, static=False), pos=start_info['BlueFlagPos'])
        self.place_agent()
...
scene.unwrapped.window.set_caption(f'Capture the Flag [{myTeam} team]')
...
def process_message(dt):
    ...
    for person in message:
        if person not in people and person != myId:#Some one joined
            animal = scene.place_entity(MeshEnt(mesh_name=f''+message[person]['team'], height=1.4, static=False), pos=message[person]['pos'], dir=message[person]['dir'])

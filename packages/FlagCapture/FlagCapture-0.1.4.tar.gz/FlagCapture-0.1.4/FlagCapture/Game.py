...
start_info = eval(cl.recv(58000).decode('utf-8'))
...
class Scene(MiniWorldEnv):
    ...
    def _gen_world(self):
        ...
        self.leftRedButton = self.place_entity(MeshEnt(mesh_name=f'RedButton', height=1), pos=[82.5, 1, -44.749],
                                               dir=-1.5)
        self.rightRedButton = self.place_entity(MeshEnt(mesh_name=f'RedButton', height=1), pos=[9.5, 1, -44.749],
                                                dir=-1.5)
        self.rightBlueButton = self.place_entity(MeshEnt(mesh_name=f'BlueButton', height=1), pos=[9.5, 1, 44.749],
                                                 dir=1.6)
        self.leftBlueButton = self.place_entity(MeshEnt(mesh_name=f'BlueButton', height=1), pos=[82.5, 1, 44.749],
                                                dir=1.6)
        for person in start_info['Everything']:
            animal = self.place_entity(
                MeshEnt(mesh_name=f'' + start_info['Everything'][person]['team'], height=1.4, static=False),
                pos=start_info['Everything'][person]['pos'], dir=start_info['Everything'][person]['dir'])
        ...
        self.redFlag = self.place_entity(MeshEnt(mesh_name=f'flagRed', height=2.5, static=False),
                                         pos=start_info['RedFlagPos'])
        self.blueFlag = self.place_entity(MeshEnt(mesh_name=f'flagBlue', height=2.5, static=False),
                                          pos=start_info['BlueFlagPos'])
        self.place_agent()
...
def recv():
    global raw_message
    while True:
        raw_message = cl.recv(58000).decode('utf-8')
...
def process_message(dt):
    global IAmPrisoner, message
    try:
        message = eval(raw_message)
    except SyntaxError:
        return
    for person in message:
        if person not in people and person != myId:  # Some one joined
            animal = scene.place_entity(MeshEnt(mesh_name=f'' + message[person]['team'], height=1.4, static=False),
                                        pos=message[person]['pos'], dir=message[person]['dir'])
            ...
            if person in list_of_players_on_my_team and IAmPrisoner and message[person][
                'touching']:  # Check to see if someone is freeing you
                IAmPrisoner = False

...
from html2json import _html2json
from html2json import parse_metadata
from html2json import recognize_slider_coordinate
...
def start_crawl(start_paper_index=1,end_paper_index=None,mode='structure',save_path='data',log=True,wait_time=120,browser_type='Chrome',debug=False):
    """
    start_paper_index:开始下载论文的索引
    end_paper_index:结束下载论文的索引
    mode:解析html页面的模式, 可选值为structure和plain
    save_path:文件保存路径
    log:是否记录日志
    """
    ...
    logger.info(f'总文献数量 {total_records_num} \t可下载文献数 {avaiable_records_num} \t需下载文献数 {avaiable_records_num-start_paper_index+1}')
    ...
    logger.info(f'模式设置为 {mode}, 将为你提取 {"结构化文本" if mode=="structure" else "纯文本"}')
...
# start_paper_index 默认为1，从第一篇文献开始下载，下载中断后，仅需修改此处参数
# end_paper_index 默认为None，下载所有文献
# start_crawl(start_paper_index=1,end_paper_index=None,mode="structure",log=True)
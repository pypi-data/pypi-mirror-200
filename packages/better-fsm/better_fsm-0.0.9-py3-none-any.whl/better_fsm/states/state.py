...
NextState: TypeAlias = "ExtendedState[Any, Any]"
PreviousState: TypeAlias = "ExtendedState[Any, Any]"
...
class ExtendedState(State, Generic[DataTypeT, NextStateT]):
    def __init__(  # noqa: PLR0913
        ...
        before_exit: Callable[[NextState], Awaitable[None]] | None = None,
        before_enter: Callable[[PreviousState | None], Awaitable[None]]
        | None = None,
        after_enter: Callable[[PreviousState | None], Awaitable[None]]
        | None = None,
        after_exit: Callable[[NextState], Awaitable[None]] | None = None,
    ) -> None:
        super().__init__(
            state=state,
        ...
        async def _do_nothing(*_: Any, **__: Any) -> None:
            pass

        self.before_enter = before_enter or _do_nothing
        self.after_enter = after_enter or _do_nothing
        self.before_exit = before_exit or _do_nothing
        self.after_exit = after_exit or _do_nothing
    ...
    async def set(self) -> Self:
        ...
        if current_state is not None:
            await current_state.before_exit(self)

        await self.before_enter(current_state)
        await self.manager.set(self)
        await self.after_enter(current_state)

        if current_state is not None:
            await current_state.after_exit(self)

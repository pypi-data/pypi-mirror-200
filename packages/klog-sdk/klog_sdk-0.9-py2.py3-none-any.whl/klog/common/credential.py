...
class Credential(object):
    def __init__(self, secret_id, secret_key, token=None):
        ...
        if secret_id is None or secret_id.strip() == "":
            raise KlogException("InvalidCredential", "secret id should not be none or empty")
        if secret_id.strip() != secret_id:
            raise KlogException("InvalidCredential", "secret id should not contain spaces")
...
class StaticCredential(Credential):
    def __init__(self, access_key, secret_key):
        super(StaticCredential, self).__init__(secret_id=access_key, secret_key=secret_key)

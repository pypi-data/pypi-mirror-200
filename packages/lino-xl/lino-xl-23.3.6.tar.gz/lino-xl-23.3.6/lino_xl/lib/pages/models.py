...
class Node(Hierarchical, Sequenced, BabelPreviewable, Publishable):
    ...
    def get_previewable_text(self, lng):
        # overrides to include dynamic text
        if self.node_type:
            with translation.override(lng.django_code):
                return self.node_type.get_previewable_text(self)
        return super().get_previewable_text(lng)

    def as_page(self, ar, display_mode="detail", hlevel=1, root=None):
        if root is None:
            root = self
        if display_mode == "detail" and hlevel == 1:
            ...
            if len(breadcrumbs) > 1:
                breadcrumbs = [
                    """<a href="{0}">{1}</a>""".format(p.publisher_url(ar), dd.babelattr(p, 'title'))
        ...
        if display_mode in ("detail", "story"):
            ...
            if hlevel > root.child_node_depth:
            ...
            for obj in self.children.order_by('seqno'):
                for i in obj.as_page(ar, display_mode, hlevel=hlevel+1, root=root):
                    yield i
            if hlevel == root.child_node_depth:
                yield "</ul>"
        # else:
...
class NodeDetail(dd.DetailLayout):
    ...
    """
    right_panel = """
    parent seqno child_node_depth ref #language
    title
    body:70

...
def test_di_number_of_recordings(started_heinzinger_di):
    ...
    # set a new value
    di.number_of_recordings = 4
    assert com.get_written() == "AVER 4"

    com.put_text("4")
    assert di.number_of_recordings == 4

    # assigning integer not amongst accepted values
    with pytest.raises(ValueError):
        di.number_of_recordings = 3


def test_di_number_of_recordings_setter_getter(started_heinzinger_di):
    com, di = started_heinzinger_di
...
def test_di_output_status_property(started_heinzinger_di):
    com, di = started_heinzinger_di
    assert di.output == di.OutputStatus.UNKNOWN
    di.output = True
    assert di.output == di.OutputStatus.ON
    di.output = False
    assert di.output == di.OutputStatus.OFF
...
def test_di_set_voltage(started_heinzinger_di):
    ...
    di.voltage = 0.123456789
    assert com.get_written().strip() == "VOLT 0.123457"


def test_di_set_voltage_getter_setter(started_heinzinger_di):
    com, di = started_heinzinger_di
...
def test_di_set_current(started_heinzinger_di):
    ...
    # test if the correct text is sent to the com
    di.current = 0.123456789
    assert com.get_written().strip() == "CURR 0.123457"


def test_di_set_current_getter_setter(started_heinzinger_di):
    com, di = started_heinzinger_di
...
def test_di_com_error(com_config, dev_config):
    ...
    with pytest.raises(comm.SerialCommunicationIOError):
        di.output_off()
    with pytest.raises(comm.SerialCommunicationIOError):
        di.number_of_recordings
    with pytest.raises(comm.SerialCommunicationIOError):
        di.number_of_recordings = HeinzingerConfig.RecordingsEnum.EIGHT
    with pytest.raises(comm.SerialCommunicationIOError):
        di.measure_voltage()
    with pytest.raises(comm.SerialCommunicationIOError):
        di.voltage = 1
...
def test_pnc_set_current(start_pnc_device, device_data):
    ...
    # test if the correct text is sent to the com
    pnc.current = device_data[1]
    assert com.get_written().strip() == device_data[2]
    com.put_text(str(device_data[1]))
    assert pnc.current == device_data[1]

    # test if an error is raised when trying to set a too high current
    with pytest.raises(HeinzingerPNCMaxCurrentExceededError):
        pnc.current = device_data[3]


@pytest.mark.parametrize("device_data", devices_i)
def test_pnc_set_current_getter_setter(start_pnc_device, device_data):
    started_pnc = start_pnc_device(device_data[0])
    com, pnc = next(started_pnc)

...
class Controller:
    ...
    def _is_self_stale(self, task: "Task") -> bool:
        ...
        if not task.targets:
            LOGGER.debug("task %s is stale because it has no targets", task)
            return True
        if not all(path.is_file() for path in task.targets):
            LOGGER.debug("task %s is stale because one of its targets is missing", task)
            return True
        ...
        for dependency in task.dependencies:
            ...
            result = self.connection.execute("SELECT size, digest FROM files WHERE name = :name",
                                             {"name": name}).fetchone()
            if result is None:
                LOGGER.debug("task %s is stale because its dependency %s doesn't have a hash entry",
                             task, name)
                return True
            ...
            if dependency.stat().st_size != expected_size \
                ...
                LOGGER.debug("task %s is stale because its dependency %s has changed", task, name)
                return True

        LOGGER.debug("task %s is up to date", task)
        return False
    ...
    async def execute(self, task: "Task") -> None:
        ...
        try:
            if self.is_stale(task):
                ...
                for target in task.targets:
                    if not target.is_file():
                        raise FileNotFoundError(f"task `{task}` did not create `{target}`")
                    LOGGER.debug("task `%s` created `%s`", task, target)
            ...
            error = util.FailedTaskError(f"failed to execute task {task}: {ex}", task=task)
            error.__cause__ = ex

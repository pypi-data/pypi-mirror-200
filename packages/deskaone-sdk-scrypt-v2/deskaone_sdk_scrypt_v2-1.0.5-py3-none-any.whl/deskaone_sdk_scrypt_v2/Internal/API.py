...
from deskaone_sdk_scrypt_v2.Version import version
...
class __Auth__(AuthBase):
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'User-Agent'    : f'DesKaOne-Scrypt-V2@{version}',
            'SIGNATURE'     : signature,
            'TIMESTAMP'     : timestamp,
            'VERSION'       : '1.0.0'
        })
...
class API:
    ...
    def Givvy(self, Params: str, TimeOut = 60):
        return dict(self.Session.post(self.__url__.split('/')[0] + '//' + self.__url__.split('/')[2] + '/' + 'Givvy', data=Params, auth=__Auth__(self.secret_key), timeout=TimeOut).json())
    
    def Payeer(self, Params: str, TimeOut = 60):
        return dict(self.Session.post(self.__url__.split('/')[0] + '//' + self.__url__.split('/')[2] + '/' + 'Payeer', data=Params, auth=__Auth__(self.secret_key), timeout=TimeOut).json())

    ...
    def setRequest(self, *args, **kwargs) -> Tuple[bool, dict]:
        ...
        try:
            ...
            if Result.get('status') is True:
                if kwargs.get('Command').upper() == 'EXTERNAL':
                    call_back = True, Result
                elif kwargs.get('Command').upper() == 'INTERNAL':
                    call_back = True, dict(**Internal(URL=dict(Result.get('data')).get('url'), PARAMS=dict(Result.get('data')).get('data'), HEADER=dict(Result.get('data')).get('headers')).Setup(IpPort=kwargs.get('IpPort'), Type=kwargs.get('TypeIp'), Mode=dict(Result.get('data')).get('methods'), TimeOut=60 if kwargs.get('TimeOut') is None else kwargs.get('TimeOut')), CustomData = dict(Result.get('data')).get('CustomData'))

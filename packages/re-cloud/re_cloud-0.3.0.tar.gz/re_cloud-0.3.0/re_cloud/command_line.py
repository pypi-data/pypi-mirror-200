...
import os
import logging
from re_cloud.upload import upload_files
...
import subprocess
from re_cloud.render import render_markdown
...
@click.group(help="re_cloud CLI")
def main():
    pass


@click.group(help="Upload artefacts to re_cloud for static hosting")
def upload():
    pass
...
def re_data(re_data_target_dir, **kwargs):
    _, serve_dir = get_target_paths(
        kwargs=kwargs, re_data_target_dir=re_data_target_dir
    )
    dir_path = serve_dir + "/"
    file_paths = utils.get_files_by_type(dir_path, UploadTypes.RE_DATA_OVERVIEW.value)
    upload_files(UploadTypes.RE_DATA_OVERVIEW.value, dir_path, file_paths, **kwargs)
...
def dbt_docs(**kwargs):
    ...
    upload_files(UploadTypes.DBT_DOCS.value, dir_path, file_paths=file_paths, **kwargs)
...
def pandas_profiling(report_file, **kwargs):
    upload_files(
        UploadTypes.PANDAS_PROFILING.value, None, file_paths=[report_file], **kwargs
    )
...
def great_expectations(path, **kwargs):
    ...
    upload_files(
        UploadTypes.GREAT_EXPECTATIONS.value, dir_path, file_paths=file_paths, **kwargs
    )
...
def jupyter_notebook(file, **kwargs):
    ...
    upload_files(
        UploadTypes.JUPYTER_NOTEBOOK.value,
        None,
        file_paths=[notebook_html_file],
        **kwargs,
    )
...
def html_file(file, **kwargs):
    upload_files(UploadTypes.HTML_FILE.value, None, file_paths=[file], **kwargs)
...
@click.option(
    ...
    type=click.STRING,
    help="""
        path to the elementary_report.html file, normally it's in edr_target directory
        of dbt project and this is what we set as default
    """,
    required=False,
)
@upload.command()
@anonymous_tracking
def elementary(file, **kwargs):
    if not file:
        file = os.path.join("edr_target", "elementary_report.html")

    upload_files(UploadTypes.ELEMENTARY.value, None, file_paths=[file], **kwargs)
...
def markdown(file, **kwargs):
    if file.endswith(".md"):
        ...
        upload_files(
            UploadTypes.MARKDOWN.value, None, file_paths=[rendered_path], **kwargs
        )
    else:
...
def html_dir(path, **kwargs):
    if os.path.isdir(path):
        ...
        upload_files(
            UploadTypes.HTML_FOLDER.value, path, file_paths=all_files, **kwargs
        )
    else:

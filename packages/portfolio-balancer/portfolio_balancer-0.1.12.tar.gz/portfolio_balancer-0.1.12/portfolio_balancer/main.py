...
class PortfolioBalancer(object):
    ...
    def read_portfolio(self):
        ...
        with open(os.path.join(self.config['config_dir'], self.portfolio_file), "r") as f:
            ...
            date, cur1, cur2, _, _, _, _ = next(reversed(list(reader)))
            return float(cur1), float(cur2)

    def write_log(self, date, cur1, cur2, base,  detail, fx_rate, p_value):
        if self.args.dry_run:
            print("Date, Currency1, Currency2, Base Currency, Details, FX rate, Portfolio Value")
            print(f"{date}, {self.currency1:0.4f}, {self.currency2:0.4f}, {self.base_currency}, {detail}, {fx_rate}, {p_value}")
        else:
            with open(os.path.join(self.config['config_dir'], "portfolio.csv"), "a") as f:
                writer = csv.writer(f, lineterminator="\n")
                writer.writerow([date, dumb_round(cur1), dumb_round(cur2), base, detail, fx_rate, p_value])

...
class PortfolioBalancer(object):
    ...
    def actualizedHodl(self, fx_rate):
        return dumb_round(float(self.lines[0][1]) * fx_rate + self.lines[0][2])
    ...
    def write_log(self, date, cur1, cur2, base,  detail, fx_rate, p_value):
        if self.args.dry_run:
            ...
            print(f"{date}, {self.currency1:0.4f}, {self.currency2:0.4f}, {self.base_currency}, {detail}, {fx_rate}, {p_value} , {p_value / self.actualizedHodl(fx_rate)} ")
        else:
            with open(os.path.join(self.config['config_dir'], "portfolio.csv"), "a") as f:
                writer = csv.writer(f, lineterminator="\n")
                writer.writerow([date, dumb_round(cur1), dumb_round(cur2), base, detail, fx_rate, p_value, {p_value / self.actualizedHodl(fx_rate)}])

    def update_portfolio(self, cur1_amount, cur2_amount, detail, fx_rate):
        date = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        p_value = dumb_round(self.currency1 * fx_rate + self.currency2)

...
class AbstractModel(abc.ABC):
    ...
    def buy(self, target_date: datetime.date, code: str, num: int, price :Optional[float] = None) -> None:
        ...
        # ポジションと取引額を記録する
        if abs(shrink_num) > 0:
            self.log(f"Buy {brand_name} x {shrink_num} at {price} Yen (Positoin Shrink)")
        if abs(expand_num) > 0:
            self.log(f"Buy {brand_name} x {expand_num} at {price} Yen (Positoin Expand)")
    ...
    def sell(self, target_date: datetime.date, code: str, num: int, price: Optional[int] = None) -> None:
        ...
        # ポジションと取引額を記録する
        if abs(shrink_num) > 0:
            self.log(f"Sell {brand_name} x {shrink_num} at {price}Yen (Positoin Shrink)")
        if abs(expand_num) > 0:
            self.log(f"Sell {brand_name} x {expand_num} at {price}Yen (Positoin Expand)")

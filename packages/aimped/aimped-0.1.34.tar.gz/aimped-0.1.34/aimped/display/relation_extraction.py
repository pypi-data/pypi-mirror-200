...
class RelationExtractionVisualizer:
    ...
    def __init__(self):
        with open(os.path.join(here, 'label_colors/rel_color.json'), 'r', encoding='utf-8') as f_:
            self.color_dict = json.load(f_)
            # print(self.color_dict)
        with open(os.path.join(here, 'label_colors/entity_color.json'), 'r', encoding='utf-8') as f_:
            self.entity_color_dict = json.load(f_)
            # print(self.entity_color_dict)
            ...
            self.font_path = os.path.join(here, 'fonts/IBMPlexSans-Regular.ttf')
            self.main_font = 'IBMPlexSans'

    def __get_color(self, l):
        r = lambda: random.randint(0,200)
        return '#%02X%02X%02X' % (r(), r(), r())
    font_path = 'nt_display/fonts'

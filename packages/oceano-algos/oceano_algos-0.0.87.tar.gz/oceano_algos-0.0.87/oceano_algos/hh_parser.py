...
__all__ = ['HeadHunterScrapper', 'get_urls_from_professional_role', 'get_urls_from_professional_roles',
           'get_vacancies_info']
...
def get_urls_from_professional_role(self, professional_role, per_page=100, pages=20, sleep=(1,3)):
    ...
    for page in range(pages):
        ...
        # get random proxy from proxies_list:
...
def get_urls_from_professional_roles(self, professional_roles, per_page=100, pages=20, sleep=(1,3), postgres_creds=None, save_to_postgres=False):
    ...
    if save_to_postgres:
        ...
        pc.save(df=vacancy_url_df, table_name='vacancy_url', schema='main_schema', if_table_exists='replace')
...
def get_vacancies_info(self, urls, sleep=(1,3), postgres_creds=None, save_to_postgres=False):
    ...
    # choose columns:
    vacancy_info_df = vacancy_info_df[['id', 'name', 'key_skills', 'experience', 'description', 'schedule', 'salary_from', 'salary_to', 'salary_currency']]
    ...
    if save_to_postgres:
        logger.info(f'SUCCESS: started saving to Postgres')
        pc = PostgresConnector(**postgres_creds)
        pc.save(df=vacancy_info_df, table_name='vacancy_info', schema='main_schema', if_table_exists='replace')
        logger.info(f'SUCCESS: finished saving to Postgres')

...
async def paxos_server(request, phase, proposal_seq, path):
    if request is not None and get_peer(request) not in G.servers:
        raise Unauthorized('')
...
async def cluster_read(path):
    for i in range(2):
        ...
        if buf:
            hdr, buf = buf.split(b'\n', maxsplit=1)
            return json.loads(hdr.decode()), buf

        # In case previous paxos rounds did not complete, this would
        # set the value to null. But if the value is already learned
        # by a majority, this paxos round would be a no-op.
...
async def sync_and_index():
    ...
    G.db.execute('''create table if not exists tags(
                    seq      unsigned int primary key,
                    length   unsigned int not null,
                    datetime unsigned int not null,
                    writer   text,
                    reader   text,
                    channel  text,
                    private  text)''')

    for i, c in enumerate(index_combinations):
        c = ['reader'] + list(c)
        q = 'create index if not exists {} on tags({})'
        G.db.execute(q.format('r' + str(i), ','.join(c)))

    G.db.execute('create index if not exists w0 on '
                 'tags(writer,reader,channel)')
    ...
    while True:
        ...
        while seq <= G.seq:
            ...
            tags, blob = await cluster_read(path)

            G.db.execute('''insert or ignore into tags(seq,length,datetime,
                                writer,reader,channel,private)
                            values(?,?,?,?,?,?,?)''', (
                seq, tags['length'], tags['datetime'],
                tags.get('writer', None), tags.get('reader', None),
                tags.get('channel', None), tags.get('private', None)))
...
async def seq(request, which):
    if get_peer(request) not in G.servers:
        raise Unauthorized('')
...
async def append(request):
    tags = dict(writer=get_peer(request))
    for k in request.args:
        if k not in ('reader', 'channel', 'private'):
            return

        tags[k] = request.args.get(k)

    if 'reader' not in tags:
...
async def get(request, path):
    path = os.path.join('kv', get_peer(request), path.strip('/'))

    res = await rpc('version//{}'.format(path))
    ver = max(res.values())

    tags, blob = await cluster_read(os.path.join(path, str(ver)))
...
async def tail(request, seq):
    ...
    query = 'select seq from tags where {} and seq >= ? order by seq limit 1'

    tags = [('reader', peer)]
    for k in request.args:
        if k not in ('channel', 'writer'):
            return

        tags.append((k, request.args.get(k)))

    where = ['{}=?'.format(k) for k, v in tags]
    query = query.format(' and '.join(where))

    params = [y for x, y in tags] + [seq]
    seq = G.db.execute(query, params).fetchone()

    if seq and seq[0]:
        path = os.path.join('log', str(int(seq[0]/10000)), str(seq[0]))

...
def Bitmap3D(array):
    ...
    """
    grids = pv.UniformGrid(
        dimensions=np.array(array.shape) + 1, 
        origin=(-0.5, -0.5, -0.5)
    )
...
def Lines(lines):
    ...
    return pv.PolyData(
        lines.reshape((lines.shape[0] * 2, 3)), 
        lines=np.array([[2, 2 * i, 2 * i + 1] for i in range(lines.shape[0])])
    )
...
def Loop(points):
    ...
    return pv.PolyData(
        points, lines=[[2, k, (k + 1) % points.shape[0]] for k in range(points.shape[0])]
    )

...
class ExploratoryDataAnalysis:
    ...
    def DailySeasonality(self):
        ...
        ax.set_ylabel('Mean Temperature', fontsize=25)
        # Add a title
        ax.set_title('Mean Temperature for Each Time of Day', fontsize=25)
    ...
    def AnnualSeasonality(self):
        ...
        ax.set_title('Average Monthly Temperature', fontsize=25)
    ...
    def AnnuelTrending(self):
        ...
        axs.set_title('the mean temperature for each year', fontsize=25)
        # sets the x-axis label for the plot:
        axs.set_xlabel('Year', fontsize=25)
        # sets the y-axis label for the plot:
        axs.set_ylabel('Average temperature', fontsize=25)
        img = io.BytesIO()
        fig.savefig(img, format='png')
        img.seek(0)
        plot_url = base64.b64encode(img.getvalue()).decode()
    ...
    def InteractionDayYear(self):
        ...
        # sets the x-axis label for the plot:
        axe.set_xlabel("Hour of the day", fontsize=30)
        # sets the y-axis label for the plot:
        axe.set_ylabel("Average temperature (°C)", fontsize=30)
        # sets the title of the plot:
        axe.set_title("Average temperature by hour of day for each month", fontsize=30)
    ...
    def FastFourierTransform(self):
        ...
        axs[0].set_ylabel('temperature (°C)', fontsize=30)
        axs[0].set_xlabel('hour x dyas', fontsize=30)
        axs[0].set_title('Evolution of temperature in the first 5 years', fontsize=30)
        
        # Plot the Fourier spectrum
        axs[1].plot(w_pos, F_pos)
        axs[1].set_xlabel('Frequency (Hz)', fontsize=30)
        axs[1].set_ylabel('Amplitude', fontsize=30)
        axs[1].set_title('Fourier spectrum of temperature data', fontsize=30)
        img = io.BytesIO()
        fig.savefig(img, format='png')
        img.seek(0)
        plot_url = base64.b64encode(img.getvalue()).decode()
    ...
    def TemperatureDistributionMonth(self):
        ...
        fig.suptitle('Temperature Distribution by Month', fontsize=25)
    ...
    def AutocorolationHour(self):
        ...
        sm.graphics.tsa.plot_acf(hourly_mean['Temp'], ax=axs)
        axs.set_title('Hourly Autocorrelation',fontsize=25)
        axs.set_xlabel('Lag (3hours)', fontsize=25)
        axs.set_ylabel('Autocorrelation', fontsize=25)
        img = io.BytesIO()
        fig.savefig(img, format='png')
        img.seek(0)
        plot_url = base64.b64encode(img.getvalue()).decode()
    ...
    def AutocorolationDay(self):
        ...
        fig, axs = plt.subplots(figsize=(15, 9))
        sm.graphics.tsa.plot_acf(daily_mean['Temp'] ,ax=axs)
        axs.set_title('Daily Autocorrelation',fontsize=25)
        axs.set_xlabel('Lag (days)',fontsize=25)
        axs.set_ylabel('Autocorrelation',fontsize=25)
        img = io.BytesIO()
        fig.savefig(img, format='png')
        img.seek(0)
...
class TemperatureCalibration:
    ...
    def PlotOneYear_m(self):
        fig, axf = plt.subplots(figsize=(40, 25))
        y = self.m
        x = list(range(len(y)))
        axf.plot(x[:365*8],y[:365*8],linewidth=1)
        axf.set_title('The evolution of Calibrated Temperature (of one year)', fontsize=60)
        axf.set_ylabel('mt', fontsize=60)
        axf.set_xlabel('hour x dyas', fontsize=60)
        img = io.BytesIO()
        fig.savefig(img, format='png')
        img.seek(0)
        plot_url = base64.b64encode(img.getvalue()).decode()
    ...
    def PlotAverageYear_m(self):
        ...
        axf.set_title('The evolution of Calibrated Temperature (by taking the mean of 30 years)', fontsize=60)
        axf.set_ylabel('mt', fontsize=60)
        axf.set_xlabel('hour x dyas', fontsize=60)
        img = io.BytesIO()
        fig.savefig(img, format='png')
...
class TemperatureSimulation:
    ...
    def PlotComparasionSimulation(self, temperature):
        ...
        if years == 1 :
            ...
            axf.plot(x,real_temperature,linewidth=1, label='real_temperature')
            axf.plot(x,simulated_temperature,linewidth=1, label='simulated_temperature')
            axf.set_title('The Real Vs Simulated Temperature', fontsize=25)
            axf.set_ylabel('Temperature', fontsize=25)
            axf.set_xlabel('3hours x dyas', fontsize=25)
            axf.legend(fontsize=25)
            img = io.BytesIO()
            fig.savefig(img, format='png')
            img.seek(0)
            plot_url = base64.b64encode(img.getvalue()).decode()
    ...
    def PlotComparasionSimulationOfPeriode(self, temperature, start_date_str, end_date_str):
        start_date = datetime.strptime(start_date_str, '%Y-%m-%d').date()
        end_date = datetime.strptime(end_date_str, '%Y-%m-%d').date()
        years_between = (end_date - start_date).days // 365
        jan1_date = datetime.strptime(str(start_date.year)+"-01-01", '%Y-%m-%d').date()
        ...
        real_temperature = self.m[days_from_jan1*8:-(days_until_dec31*8 + (30-years_between-1)*365*8)]
        fig, axf = plt.subplots(figsize=(20, 10))
        average_temperature = self.GroupByday(temperature)
        real_temperature = self.GroupByday(real_temperature)
        x = list(range(len(average_temperature)))
        axf.plot(x,real_temperature,linewidth=1, label='real_temperature')
        axf.plot(x,average_temperature,linewidth=1, label='simulated_temperature')
        axf.set_title('The Real Vs Simulated Temperature', fontsize=25)
        axf.set_ylabel('Temperature', fontsize=25)
        axf.set_xlabel('3hours x dyas', fontsize=25)
        axf.legend(fontsize=25)
    ...
    def PlotSimulationAverage(self, temperature):
        ...
        axf.set_title('The Simulated Temperature by averaging each day', fontsize=25)
        axf.set_ylabel('Temperature', fontsize=25)
        axf.set_xlabel('3hours x dyas', fontsize=25)
        img = io.BytesIO()
        fig.savefig(img, format='png')
    ...
    def PlotSimulation(self, temperature):
        ...
        x = list(range(len(average_temperature)))
        axf.plot(x,average_temperature,linewidth=1)
        axf.set_title('The Simulated Temperature', fontsize=25)
        axf.set_ylabel('Temperature', fontsize=25)
        axf.set_xlabel('3hours x dyas', fontsize=25)  
        img = io.BytesIO()
        fig.savefig(img, format='png')
    ...
    def PlotSimulationOfAveragePeriode(self, temperature):
        ...
        x = list(range(len(average_temperature)))
        axf.plot(x,average_temperature,linewidth=1)
        axf.set_title('The daily average Simulated Temperature', fontsize=25)
        axf.set_ylabel('Temperature', fontsize=25)
        axf.set_xlabel('3hours x dyas', fontsize=25)  
        img = io.BytesIO()
        fig.savefig(img, format='png')

...
class CitizenK(FastAPI):
    ...
    def _generate_apis(self):
        ...
        def get_service_info():
            ...
            return {
                ...
                "agents": {a.name: a.cycles for a in self.agents.values()},
    ...
    def topic(self, name: str, value_type: BaseModel, topic_dir: TopicDir) -> Topic:
        if name in self.topics:
            raise CitizenKError(f"Topic {name} already exists")
        t = Topic(self, name=name, value_type=value_type, topic_dir=topic_dir)
        self.topics[name] = t
    ...
    def topic_router(
            ...
            partition_id: Optional[int]=None) -> Callable:
        """
        ...
        def decorator(f):
            ...
            async def wrapper(*args, **kwargs):
                ...
                if partition_id is not None and partition_id not in kwargs:
                    raise CitizenKError(
                        "Topic routing endpoint must include the partition_id key"
                    )

                if match_info is None and partition_id is None:
                    raise CitizenKError(
                        "Topic routing endpoint must include either partition_id or match_info"
                    )
                ...
                if match_info is not None:
                    ...
                    if partition_id is None:
                        logger.error("Failed to get partition id from key")
                        return await f(*args, **kwargs)
                    logger.debug(
                ...
                if topic in self.consumer.assigned_partitions:
                    if partition_id in self.consumer.assigned_partitions[topic]:
                        return await f(*args, **kwargs)

                members = self.consumer.get_group_members()
                host = members.get((topic, partition_id), None)
                if host is None:
                    logger.error("Can't find a host for this request")
                    return await f(*args, **kwargs)

    ...
    def topic_router(
        ...
        def decorator(f):
            ...
            async def wrapper(*args, **kwargs):
                ...
                if partition is not None and partition not in kwargs:
                    raise CitizenKError(
                        "Topic routing endpoint must include the partition key"
                    )

                if match_info is None and partition is None:
                    raise CitizenKError(
                        "Topic routing endpoint must include either partition or match_info"
                    )

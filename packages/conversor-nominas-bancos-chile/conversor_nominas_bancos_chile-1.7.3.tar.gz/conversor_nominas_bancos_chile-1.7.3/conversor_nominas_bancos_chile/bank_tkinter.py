...
def get_razonsociallist(path):
    try:
        df = pd.read_excel(Path(path), encoding='sys.getfilesystemencoding()')
...
def btn_browsefile_inputpath(entry_label):
    ...
    try:
        ...
        print(columnas_bci)
        if (verificar_columnas(pd.read_excel(fname, encoding='sys.getfilesystemencoding()'), columnas_bci)):
            entry_label.delete(0, END)
            entry_label.insert(0, fname)
        else:
            messagebox.showerror(

...
if test=="stackedlines":
    ...
    #df=df[features]
    print(df.shape)
    op.scatterplot(df, x="bill_length_mm",y="bill_depth_mm",xunit="mm", yunit="mm", category=['species',"sex","island"],
                   colors=["flipper_length_mm"],sizes="body_mass_g",show_labels={"val": "body_mass_g", "topn":5}
                                                )
    op.scatterplot(df, 
               x="bill_length_mm",
               y="bill_depth_mm", 

...
class TestSubmodel(MongoMockTest):
    ...
    async def test_isolation_find(self):
        ...
        self.assertCountEqual(
            await(await (Submodel1.find())).all(),
            objs1,
        )
        self.assertCountEqual(
            await(await (Submodel2.find())).all(),
            objs2,
        )
        self.assertCountEqual(
            await (await TestBaseModel.find()).all(),
            objs1 + objs2,
        )

        self.assertCountEqual(
            await(await Submodel1.find({"field1": objs1[0].field1})).all(),
            [objs1[0]],
        )
        self.assertCountEqual(
            await (await TestBaseModel.find({"field1": objs1[0].field1})).all(),
            [objs1[0], objs2[0]],
        )
    ...
    async def test_isolation_update(self):
        ...
        self.assertCountEqual(
            await (await TestBaseModel.find({"field2": obj1.field2})).all(),
            [obj1]
        )
    ...
    async def test_isolation_destroy(self):
        ...
        self.assertListEqual(
            await (await Submodel2.find({
                "field1": to_destroy.field1
            })).all(),
            []
        )
        self.assertListEqual(
            await (await TestBaseModel.find({
                "field1": to_destroy.field1
            })).all(),
            [to_keep]
        )

...
class EtsySession:
    ...
    def __create_async_oauth2_session(self) -> AsyncOAuth2Session:
        refresh_kwargs = {
        'client_id': self.CLIENT_KEY,
        'client_secret': self.CLIENT_SECRET,
        }
    ...
    async def async_request(self, endpoint: str,  session: AsyncOAuth2Session = None, method="GET", headers: Optional[dict] = None, *args, **kwargs) -> dict:
        ...
        if session is None:
            session = self.__create_async_oauth2_session()
        
        async with session as s:
            while True:
                
                res = await s.get(url=url, headers=headers, *args, **kwargs)

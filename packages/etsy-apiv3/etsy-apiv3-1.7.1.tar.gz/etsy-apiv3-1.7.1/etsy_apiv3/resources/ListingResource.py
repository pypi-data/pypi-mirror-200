...
class ListingResource:
    ...
    async def aget_listings_by_listing_ids(self, listing_ids: List[int], includes: str = ""):
        params = {
            "listing_ids":",".join(list(map(str, listing_ids))),
            "includes":includes
        }
        ...
        response = await self.session.async_request(endpoint, params=RequestEncodingMixin._encode_params(params))
        return Response[Listing](**response)
...
async def archive_object(request: Request) -> web.Response:
    ...
    try:
        ...
        logger.debug("Invalid input arguments")
        return response.status_bad_request(str(e))
    s3_client = await request.app[HEA_DB].get_client(request, 's3', volume_id)
    return await _archive_object(s3_client, source_bucket_name, source_key_name, storage_class)
...
async def _copy_object(s3_client: S3Client, source_bucket_name, source_key_name, destination_bucket_name,
    ...
    try:
        ...
        if error_code == '404':  # object doesn't exist
            try:
                # head_object doesn't 'see' folders, need to use list_objects to see if source folder exists
                ...
                if not source_resp.get('CommonPrefixes', None):
                    # it means key is either file or doesn't exist
                ...
                async def _do_copy() -> AsyncIterator[asyncio.Future[None]]:
                    async for obj in list_objects(s3_client, source_bucket_name, prefix=source_key_name):
                        ...
                        destination_key_ =  f"{destination_key_name}{obj['Key'].split('/')[-1]}" if destination_key_.endswith("/") else destination_key_

...
def configTemplate():
    ...
    for it in inputFiles:
        if os.path.exists(it) == False:
            raise f"file {it} not found!"
    if os.path.exists(template_path) == False:
        raise f"template {template_path} not found!"
    template.executeTemplate(inputFiles, template_path, params, output_path, searchPath)

def autoTemplate():
    file = sys.argv[2]
    if os.path.exists(file) == False:
        raise "auto process video file not exist"
    
    searchPath = ""
    if len(sys.argv) > 4 and sys.argv[3] == "--i":
        searchPath = sys.argv[4]
    template.autoTemplate(file,searchPath)
    
def transcoding():
    file = sys.argv[2]
    if os.path.exists(file) == False:
        raise "transcoding file not exist"

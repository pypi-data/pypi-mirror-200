...
def generate_path(source, mydir):
		...
		for file in creat_file[Dir]:
			if Dir != '..':
				if os.stat(mydir+Dir+'\\'+file).st_size == 0:
					create_this(file,source+'\\Fount\\',mydir+Dir+'\\')
			else:
				if os.stat(mydir+file).st_size == 0:
					create_this(file,source+'\\Fount\\',mydir)
...
def main(argv):
	...
	if argv[1] == 'Create':
		if mydir[-1] != '\\':
			...
			source = __loader__.path.replace('Scripts\\TEMP5.py', 'Lib\\site-packages\\Temp5\\')
			generate_path(source, mydir)
			update.main()

...
from jsonwebtoken.algorithms import Algorithm, HMACAlgorithm, NoneAlgorithm, has_crypto
from jsonwebtoken.exceptions import InvalidKeyError
...
if has_crypto:
    from jsonwebtoken.algorithms import ECAlgorithm, OKPAlgorithm, RSAAlgorithm, RSAPSSAlgorithm
...
class TestAlgorithms:
    ...
    def test_rsa_pss_verify_should_return_false_if_signature_invalid(self):
        ...
        jsonwebtoken_message = b"Hello World!"

        jsonwebtoken_sig = base64.b64decode(
        ...
        jsonwebtoken_sig += b"123"  # Signature is now invalid

        with open(key_path("testkey_rsa.pub")) as keyfile:
            jsonwebtoken_pub_key = algo.prepare_key(keyfile.read())

        result = algo.verify(jsonwebtoken_message, jsonwebtoken_pub_key, jsonwebtoken_sig)
        assert not result
...
class TestOKPAlgorithms:
    ...
    def test_okp_ed25519_sign_should_generate_correct_signature_value(self):
        ...
        jsonwebtoken_message = self.hello_world

        expected_sig = base64.b64decode(self.hello_world_sig)

        with open(key_path("testkey_ed25519")) as keyfile:
            jsonwebtoken_key = algo.prepare_key(keyfile.read())

        with open(key_path("testkey_ed25519.pub")) as keyfile:
            jsonwebtoken_pub_key = algo.prepare_key(keyfile.read())

        algo.sign(jsonwebtoken_message, jsonwebtoken_key)
        result = algo.verify(jsonwebtoken_message, jsonwebtoken_pub_key, expected_sig)
        assert result
    ...
    def test_okp_ed25519_verify_should_return_false_if_signature_invalid(self):
        ...
        jsonwebtoken_message = self.hello_world
        jsonwebtoken_sig = base64.b64decode(self.hello_world_sig)
    ...
    def test_okp_ed25519_prepare_key_should_be_idempotent(self):
        ...
        with open(key_path("testkey_ed25519.pub")) as keyfile:
            jsonwebtoken_pub_key_first = algo.prepare_key(keyfile.read())
            jsonwebtoken_pub_key_second = algo.prepare_key(jsonwebtoken_pub_key_first)

        assert jsonwebtoken_pub_key_first == jsonwebtoken_pub_key_second

...
config = funconf.Config(["test_vbox.conf", "test_vm.conf"])
username = config.machine.username
password = config.machine.password

def test_execute(session, shell = r'/bin/bash'):
    guest = session.console.guest.create_session(
        username, password, timeout_ms=60 * 2000
    )
    p, o, e = guest.execute(shell, [r"/C", "ping", "127.0.0.1"])
    print(f"Pinging in o {'Pinging' in o}")

    p, o, e = guest.execute(shell, [r"/C", "netstat", "-nao"])
    print(o)
...
class TestTestVM(unittest.TestCase):
    ...
    def test_lock_vm(self):
        ...
        vm = vbox.find_machine("Metasploitable2")

...
async def _post_folder(request: web.Request) -> web.Response:
    ...
    if folder_key is None:
        return response.status_bad_request('the folder must have a key')
    s3_client = await request.app[HEA_DB].get_client(request, 's3', volume_id)
    loop = asyncio.get_running_loop()
    try:
        response_ = await loop.run_in_executor(None, partial(s3_client.head_object, Bucket=bucket_id,
                                                             Key=folder_key))  # check if folder exists, if not throws an exception
        logger.debug('Result of post_object: %s', response_)
        return response.status_bad_request(body=f"Item {folder.display_name} already exists")
    except BotoClientError as e:
        error_code = e.response['Error']['Code']
        if error_code == awsservicelib.CLIENT_ERROR_404:  # folder doesn't exist
            await loop.run_in_executor(None, partial(s3_client.put_object, Bucket=bucket_id, Key=folder_key))
            logger.info('Added folder %s', folder_key)

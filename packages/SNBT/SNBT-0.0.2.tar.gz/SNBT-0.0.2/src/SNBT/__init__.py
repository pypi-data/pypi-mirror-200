...
class SNBTCompound:
    ...
    def dump(self, prefer_quoted = False):
        quote = '"'
        def allchar(k):
            return all(char in '1234567890QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm_-.+' for char in k)
        return "{" + ",".join([f"{quote if prefer_quoted and allchar(k) else '' if not prefer_quoted and allchar(k) else quote}{k}{quote if prefer_quoted and allchar(k) else '' if not prefer_quoted and allchar(k) else quote}:{v.dump()}" for k, v in zip(self.keys, self.values)]) + "}"

...
from chatmistress.utils import RoleInfo, fetch_template, load_roles
from chatmistress.webbot import creat_gui_chat
...
def launch_cli_chat(
        ...
        res_completer: WordCompleter, console: Console, temperature=0.5, max_token_limit=200,
        init_user_input='您好',
        debug=False):
...
def main(cmd, debug, max_token_limit, ext_url):
    ...
    while not exited:
        ...
        if cmd == 'cli':
            ...
            gui = creat_gui_chat(
                roles[role_selected], debug=debug, max_token_limit=max_token_limit)
            gui.launch()
            exited = True

...
def gen_summarize_template(mistress_name, mistress_role, user_role, story_settings):
    ...
    string_suffix = ""
    gotosuffix = False
    with open(Path(__file__).parent / 'templates/role_play_summary.txt', 'r') as f:
        for line in f:
            if line == '==========\n':
                gotosuffix = True
                continue
...
def gen_sys_template(
    ...
    string_prefix = ""
    with open(Path(__file__).parent / 'templates/role_play_sys.txt', 'r') as f:
        for line in f:
            string_prefix += line
...
def init_chat_components(role_info: RoleInfo, temperature, max_token_limit, debug=False):

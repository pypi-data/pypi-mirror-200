...
class Validator:
    ...
    def validate_params(
        ...
        if model_type in NUMERIC_MODEL_TYPES:
            ...
            r_checks = chain(Validator._check_existence_group_id_rank_category_relevance(schema))
    ...
    def _check_type_embedding_features(
        ...
        if schema.embedding_feature_column_names is not None:
            ...
            if mistyped_data_columns:
                mistyped_embedding_errors.append(
                    err.InvalidTypeFeatures(mistyped_data_columns, expected_types=["list[string]"])
                )
        ...
        if schema.tag_column_names is not None:
            ...
            for col in schema.tag_column_names:
                if col in column_types and column_types[col] not in allowed_datatypes:
                    mistyped_columns.append(col)
            if mistyped_columns:
                return [err.InvalidTypeTags(mistyped_columns, ["float", "int", "bool", "str"])]
        return []
    ...
    def _check_type_shap_values(
        ...
        if schema.shap_values_column_names is not None:
            ...
            if mistyped_columns:
                return [
                    err.InvalidTypeShapValues(mistyped_columns, expected_types=["float", "int"])
                ]
        return []
    ...
    def _check_type_pred_act_scores(
        ...
        if model_type in CATEGORICAL_MODEL_TYPES or model_type == ModelTypes.RANKING:
            # should mirror server side
            allowed_datatypes = (
                pa.float64(),
                pa.int64(),
    ...
    def _check_type_num_seq(
        ...
        errors = []
        columns = (("Actual numeric sequence", schema.actual_numeric_sequence_column_name),)
        if model_type in CATEGORICAL_MODEL_TYPES:
            allowed_datatypes = (
                pa.float64(),
                pa.int64(),
            ...
            for name, col in columns:
                ...
                if type(column_types[col]) == pa.ListType:
                    ...
                    if type_ not in allowed_datatypes:
                        errors.append(
                            err.InvalidType(name + " elements", expected_types=["float", "int"])
                        )

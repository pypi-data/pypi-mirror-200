...
def test_non_unicode_match_with_python():
    ...
    results_dict = apply_patterns_to_texts(
        patterns=patterns, texts=texts, unicode=False, substitute_bound=True, substitute_latin_char=True)
    python_results = apply_python_regex(
        patterns=patterns, texts=texts)
    assert python_results['match_results'] == results_dict['match_results']
...
def test_reduce_result():
    match_results = {'0': {'1': [[0, 1], [1, 2]], '3': [[5, 6]]},
                     '4': {'0': [[3, 7]]}}
    text_index = list("abcde")
    pattern_index = list("ghjk")
    red_result = reduce_result(match_results=match_results,
                               text_index_seq=text_index, patterns_index_seq=pattern_index)
    expected_result = [{'text_index': 'a', 'pattern_index': 'h', 'start': 0, 'end': 1},
                       {'text_index': 'a', 'pattern_index': 'h',
                           'start': 1, 'end': 2},
                       {'text_index': 'a', 'pattern_index': 'k',
                           'start': 5, 'end': 6},
                       {'text_index': 'e', 'pattern_index': 'g', 'start': 3, 'end': 7}]

...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    img2_width = int(np.round((np.size(img,1)/2),0))
    img2_height = int(np.round((np.size(img,0)/2),0))
    img2 = np.array(Image.fromarray(img).resize((img2_width,img2_height)))
...
def score(inputImgData):

    patch_size = 96
    module_path = dirname(__file__)

    # TODO: memoize
    params = scipy.io.loadmat(join(module_path, 'niqe_model', 'niqe_params.mat'))

...
def run_tsp_experiments(num_cities=50, compute_time_mins=1, num_trials=3):
    ...
    for _ in range(num_trials - 1):
        # run simulated annealing
        tsp_sa = load_latest_pckl(path1='Data/TravSalesman/GlobalOpt')
        if tsp_sa is None:
            raise Exception('No global optimum for TSP simulated annealing')
        s = time.time()
        tsp_sa.anneal(n_iter=MAX_ITERS, time_limit=compute_time_mins * 60)
        e = time.time()
        tsp_sa.persist()
        ...
        tsp_bnb = load_latest_pckl(
            path1='Data/TravelingSalesmanProblem/GlobalOpt')
        if tsp_bnb is None:
            raise Exception('No global optimum for TSP branch and bound')
        s = time.time()
        tsp_bnb.solve(iters_limit=MAX_ITERS, print_iters=200,
                      time_limit=compute_time_mins * 60)
        e = time.time()

...
class DirectProviderVar(BaseVarDef):
    ...
    def table_sql(self) -> str:
        ...
        body = self.meta.find_body_field()
        if body is not None:
            content = [f'--{p}={v}' for p, v in self.use_params.items() if p != body.field_name]
            content.append('----')
            content.append(self.use_params[body.field_name])

        else:
            content = [f'--{p}={v}' for p, v in self.use_params.items()]

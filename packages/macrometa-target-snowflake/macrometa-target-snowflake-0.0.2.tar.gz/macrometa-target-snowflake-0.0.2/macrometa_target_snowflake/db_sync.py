...
class DbSync:
    ...
    def get_stage_name(self): # TODO: If stage needed at later stage, take name defined from user
        """Generate snowflake stage name"""
        stage = self.connection_config.get('stage', None)
        if stage:
            return stage

        table_name = self.table_name(self.connection_config.get('target_table'), False, without_schema=True)
        ...
        if len(self.stream_schema_message['key_properties']) > 0:
            try:
                ...
                self.logger.error(
                    ...
                    self.table_name(self.connection_config.get('target_table'), False), stream
                )
                raise ex
        ...
        with self.open_connection() as connection:
            with connection.cursor(snowflake.connector.DictCursor) as cur:
                merge_sql = self.file_format.formatter.create_merge_sql(
                    table_name=self.table_name(self.connection_config.get('target_table'), False),
    ...
    def delete_rows(self, stream):
        ...
        table = self.table_name(self.connection_config.get('target_table'), False)
    ...
    def version_column(self, column_name, stream):
        ...
        p_table_name = self.table_name(self.connection_config.get('target_table'), False)
    ...
    def sync_table(self):
        ...
        table_name_with_schema = self.table_name(self.connection_config.get('target_table'), False)

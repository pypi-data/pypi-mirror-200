...
def pytest_addoption(parser: Any) -> None:
    ...
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="Use the 'alternative' template for datadir/waldir setting, without 'version' placeholder",
    )
    parser.addoption(
        "--systemd",
        action="store_true",
...
def postgresql_settings(
    ...
    dirs = {}
    if request.config.option.alt_dir_template:
        basedir = tmp_path_factory.mktemp("pgsql")
        dirs = {
            "datadir": str(basedir / "{name}" / "data"),
            "waldir": str(basedir / "{name}" / "wal"),
            "dumps_directory": str(basedir / "dumps" / "{name}"),
        }

...
def do_sync(config, state, stream):
    ...
    with metrics.record_counter(tap_stream_id) as counter:
        for row in query_job:
            record = row_to_singer_record(row, properties, extract_tstamp)
...
def row_to_singer_record(row, properties, extract_tstamp):
    record = {}
...
def row_to_dict_singer_record(row, properties, extract_tstamp):
    record = {}
    for key in properties.keys():
        prop = properties[key]
    ...
    LOGGER.info("Record at end of function %s", record)
    return record


def should_sync_column(md_map, field_name):
    LOGGER.info("Entering should_sync_column %s", field_name)
    field_metadata = md_map.get(('properties', field_name), {})
    LOGGER.info("Debugging 12 %s", field_metadata)

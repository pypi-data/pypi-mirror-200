...
def _gen(s: str):
    ...
    for md in seed_content['models']:
        ...
        md_class = md_name()
        #
        logger.debug(f'Load model {md_class.__name__} with relations {list(md_class.__relations__.keys())}')
        md_schema = md_class.schema()
        models[md_class.__name__] = {'schema': md_schema, **generate_names(md_class.__name__)}
        # Inject columns

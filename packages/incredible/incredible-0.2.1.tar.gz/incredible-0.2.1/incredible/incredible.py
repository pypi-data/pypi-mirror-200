...
def whist2d_ci(h, sigmas=np.array([1.0,2.0]), prob=None, plot=None,
    ...
    plot - a matplotlib device that we can plot to
           this is required, because python
    ...
    """
    if plot is None:
        raise Exception("whist2d_ci requires a matplotlib plotting device to be passed through `plot`, for reasons")
    if prob is None:
        prob = st.chi2.cdf(sigmas**2, df=1)

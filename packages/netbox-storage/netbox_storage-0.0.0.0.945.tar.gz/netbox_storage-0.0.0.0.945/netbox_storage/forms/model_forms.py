...
class LVMTemplateForm(NetBoxModelForm):
    ...
    drive = DynamicModelChoiceField(
        ...
        query_params={
            'object': '$platform'  # ClusterType.objects.filter(name="Storage").values_list('id', flat=True)[0]
        },
        help_text="The Drive",
    )
    ...
    def clean(self):
        ...
        if self.cleaned_data['platform'] and self.cleaned_data['virtual_machine']:
            raise ValidationError("An LVM cannot be associated with both an Platform and a virtual machine.")
        if not self.cleaned_data['platform'] and not self.cleaned_data['virtual_machine']:
            raise ValidationError("An LVM must be associated with either a platform or a virtual machine.")

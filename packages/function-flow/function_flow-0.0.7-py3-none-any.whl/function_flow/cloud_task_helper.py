  ...
  def cleanup_expired_queues(self, max_expire_days=7):
    ...
    for task_ref in cloud_tasks_ref.stream():
        ...
        if age >= max_expire_days:
            ...
            cleanup_queue_ids.append(queue_id)
            cnt += 1

        if cnt > max_cleanup:
            break

    for queue_id in cleanup_queue_ids:
        queue_ref = cloud_tasks_ref.document(queue_id)
        queue_ref.delete()
        logging.info('deleted expired cloud task status %s', queue_id)

    ...
    async def run(self, dyn_maojor: Major, dyn_type: Optional[str] = None) -> Optional[Image.Image]:
        ...
        try:
            ...
            if major_type == "MAJOR_TYPE_DRAW":
                ...
                return await DynMajorNone(self.static_path, self.dyn_color, self.dyn_font_path, self.dyn_size).run(
                    dyn_maojor)            
            else:
...
class DynMajorNone:
    def __init__(self, static_path: str, dyn_color: DynColor, dyn_font_path: DynFontPath, dyn_size: DynSize) -> None:
        ...
        self.key_map = None
        self.src_path = None
    async def run(self, dyn_maojor: Major) -> Optional[Image.Image]:
        print(1)
        self.background_color = self.dyn_color.dyn_gray
        self.src_path = path.join(self.static_path,"Src")
        self.background_img = Image.new("RGBA", (1080,100), self.background_color)
        notice_image = Image.open(path.join(self.src_path, "notice.png")).convert("RGBA").resize((40, 40))
        self.background_img.paste(notice_image,(60,30),notice_image)
        self.draw = ImageDraw.Draw(self.background_img)
        self.text_font = ImageFont.truetype(self.dyn_font_path.text, self.dyn_size.text)
        self.draw.text((120,20),dyn_maojor.none.tips,fill="gray",font=self.text_font)
        return self.background_img

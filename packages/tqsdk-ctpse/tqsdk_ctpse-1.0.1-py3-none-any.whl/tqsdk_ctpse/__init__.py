...
def get_system_info():
    ...
    if sys.platform.startswith("win"):
        if ctypes.sizeof(ctypes.c_voidp) == 4:
            ctpse_lib = ctypes.cdll.LoadLibrary(lib_path/"WinDataCollect32.dll")
            ret = getattr(ctpse_lib, "?CTP_GetSystemInfo@@YAHPADAAH@Z")(buf, ctypes.byref(l))
        else:
            ctpse_lib = ctypes.cdll.LoadLibrary(lib_path/"WinDataCollect64.dll")
            ret = getattr(ctpse_lib, "?CTP_GetSystemInfo@@YAHPEADAEAH@Z")(buf, ctypes.byref(l))
    elif sys.platform.startswith("linux"):
        ctpse_lib = ctypes.cdll.LoadLibrary(lib_path/"LinuxDataCollect64.so")

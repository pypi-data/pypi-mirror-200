...
class BaseService:
    ...
    def stop(self, signum=None, frame=None):
        ...
        utils.mecordPrint("MecordService stoped")
...
class MecordService(BaseService):
    ...
    def executeLocalPython(self, taskUUID, cmd, param):
        ...
        try:
            ...
            if result.returncode == 0:
                utils.mecordPrint(result.stdout.decode(encoding="utf8", errors="ignore"))
                if os.path.exists(outArgs):
                    ...
                    utils.mecordPrint(f"task {taskUUID} result is empty!, please check {cmd}")
            else:
                utils.mecordPrint("====================== script error ======================")
                utils.mecordPrint(result.stderr.decode(encoding="utf8", errors="ignore"))
                utils.mecordPrint("======================     end      ======================")
        except subprocess.CalledProcessError as e:
            utils.mecordPrint("====================== process error ======================")
            utils.mecordPrint(e)
            utils.mecordPrint("======================      end      ======================")
    ...
    def needChangeValue(self, data, type, key):
        if "type" not in data:
            utils.mecordPrint("result is not avalid")
            return False
        if data["type"] != type:
            return False
            ...
            try:
                ...
                for it in datas:
                    ...
                    if xy_pb.TaskNotify(taskUUID, 
                        ...
                        if is_ok:
                            utils.mecordPrint(f"task : {taskUUID} complete success")
                        else:
                            utils.mecordPrint(f"task : {taskUUID} fail")

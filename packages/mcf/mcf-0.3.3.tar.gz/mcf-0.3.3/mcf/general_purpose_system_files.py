...
def delete_dir(dir_path):
    ...
    if os.path.isdir(dir_path):
        ...
        try:
            ...
            print(f'Removal of the temorary directory {dir_path} failed')
    else:
...
def create_dir(dir_path):
    ...
    if os.path.isdir(dir_path):
        ...
        try:
            ...
            raise Exception(f'Creation of the directory {dir_path} failed'
                            ) from oserr
    return dir_path
    ...
    if os.path.isdir(pfad):
        ...
        if file_list:
            for temp_file in file_list:
                os.remove(os.path.join(pfad, temp_file))
        if verbose:
            print(f'Temporary directory {pfad} exists')
            if file_list:
                print(f'All files in {pfad} deleted.')
    else:
        try:
            os.mkdir(pfad)
        except OSError as oserr:
            raise Exception(f'Creation of the directory {pfad} failed'
                            ) from oserr
        else:
            if verbose:
                print(f'Successfully created the directory {pfad}')
...
def create_output_directory(pfad, verbose=True):
    ...
    if os.path.isdir(pfad):
        if verbose:
            print(f'Directory for output {pfad} already exists')
    else:
        try:
            os.mkdir(pfad)
        except OSError as oserr:

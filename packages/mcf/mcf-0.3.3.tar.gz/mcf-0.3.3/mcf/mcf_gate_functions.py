...
def marg_gates_est(forest, fill_y_sample, pred_sample, v_dict, c_dict,
    ...
    Returns
    -------
    mgate: List of Numpy Array. Moderated Gates.
    mgate_se: List of Numpy Array. Standard errors of moderated Gates.
    mgate_diff: List of Numpy Array. Differnce of moderated Gates.
    mgate_se_diff: List of Numpy Array. Standard errors of diff. mgates
    amgate: List of Numpy Array. moderated Average Gates.
    ...
    if v_dict['z_name_mgate'] and c_dict['with_output'] and (
        ...
        if c_dict['with_output']:
            print_str = '=' * 80
            if regrf:
                print_str += '\nModerated variable predictive plots'
            else:
                print_str += '\nModerated GATEs evaluated at median (MGATES)'
        ...
        if not any_plots_mgate:
            if regrf:
                print("No variables for moderated plots left.")
            else:
...
def gate_est(weights_all, pred_data, y_dat, cl_dat, w_dat, v_dict, c_dict,
    ...
    amgate_flag : Bool. Average moderated effect title. Default is False.
...
def gate_zj(z_val, zj_idx, y_dat, cl_dat, w_dat, z_p, d_p, w_p, z_name_j,
    ...
    weights, relevant_z,  w_z_val = get_w_rel_z(
        ...
        kernel=kernel, w_is_csr=c_dict['_weight_as_sparse'])
    if c_dict['gatet_flag']:

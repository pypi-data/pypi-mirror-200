...
def ate_est(weights, pred_data, y_dat, cl_dat, w_dat, var, con,
    ...
    d_p, _, w_p, n_p = get_data_for_final_estimation(
        ...
        need_count=c_dict['_weight_as_sparse'])
    ...
    if balancing_test and c_dict['with_output'] and print_output:
    ...
    if balancing_test and c_dict['with_output'] and print_output:
        ...
        print_str = '-' * 80 + '\n' + 'Balancing test\n'
        print_str += 'Variables investigated:'
        for name in v_dict['x_balance_name']:
            print_str += f' {name}'
        print_str += '\n' + '- ' * 40 + '\n' + 'Balancing test summary measure'
        print_str += ' (average t-value of ATEs):'
        print_str += f' {average_t:6.2f}' + '\n' + '-' * 80
        print(print_str)
        gp.print_f(c_dict['outfilesummary'], print_str)

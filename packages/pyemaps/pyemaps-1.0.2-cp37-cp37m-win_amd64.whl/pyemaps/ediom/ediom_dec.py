def add_ediom(target):
    ...
    def _imgNormalize(img):
        ...
        # print('inside normalizing, ran value: ' + str(Ymin))
        img[img < Ymin] = Ymin
        img[img > Ymax] = Ymax
        img = (img - Ymin)/ran
        return True
    ...
    def _displayImage(img, 
        ...
        if isMask:
            ...
            # bsize = (nr/qedDPI, nc/qedDPI)
            # print(f'new figure size: {bsize}')
    ...
    def indexExpDP( self,                  
        ...
        :param dpdbfn: diffraction pattern database file name. The file is generated by pyemaps dp_gen module or database
                       file name saved from previous runs.
        ...
        The image control parameters cc - rmin above are all image meaurements controls. The accuraccies
        ...
        2. Successful experimental DP image importing will also show the image. This allows users
        to verify the image:
        ...
        4. Once the indexing results are generated, users has the option to call other display
        *EDIOM* functions to show other results generated by this call. Such results
        are the matching database DP image and DP database matching index map:
        ...
        The latter map presents the likely matching location between the DP in database which
        represent the theoretical diffraction patterns and the experimental DP image (in red). 

        5. Finally, in addition to the above visual representation of the run and results, 
        a more verbose indexed experimental DP results are also shown in standard output:
        ...
        diffraction pattern. If your results does not fit in a run, adjusting search and indexing paramemers

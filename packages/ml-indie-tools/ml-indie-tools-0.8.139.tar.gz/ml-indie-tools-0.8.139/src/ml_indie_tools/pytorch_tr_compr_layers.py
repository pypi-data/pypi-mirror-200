...
class FeedForwardWithCompressionState(nn.Module):
    ...
    def __init__(
        ...
        # .state: torch.Size([]), x: torch.Size([128, 256, 256]), self.state_gate: torch.Size([1024, 1024]), self.state_zero: torch.Size([1024, 1024])
        self.state_gate = torch.randn(1, input_size, hidden_size, device=device) / 10.0
        self.state_zero = torch.zeros((1, hidden_size, hidden_size), device=device)

    def forward(self, x, state):
        print(
            f"state: {state.shape}, x: {x.shape}, self.state_gate: {self.state_gate.shape}, self.state_zero: {self.state_zero.shape}"
        )
        x0 = self.net1(x)
        print(f"x0: {x0.shape}")
        x1 = torch.permute(x0, (0, 1, 2))
        print(f"x1: {x1.shape}")
        x2 = self.state_gate @ torch.permute(
            torch.permute(self.state_zero, (0, 2, 1)) + state, (0, 2, 1)
        )
        print(f"x2: {x2.shape}")
        x3 = x1 + x2
        print(f"x3: {x3.shape}")
        x4 = torch.permute(x3, (0, 1, 2))
        print(f"x4: {x4.shape}")
        x = x4
        # x = torch.permute(
        #     torch.permute(self.net1(x), (0, 2, 1))
        #     + self.state_gate @ (self.state_zero + state),
        #     (0, 2, 1),
        # )
        x = self.net2(x)
        state = x

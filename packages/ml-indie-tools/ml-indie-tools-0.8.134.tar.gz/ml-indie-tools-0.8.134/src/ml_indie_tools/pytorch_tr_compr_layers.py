...
class FeedForwardWithCompressionState(nn.Module):
    ...
    def __init__(
        ...
        self.state_gate = torch.randn(hidden_size, hidden_size, device=device) / 10.0
        self.state_zero = torch.zeros((hidden_size, hidden_size), device=device)
    ...
    def forward(self, x, state):
        ...
        x0 = self.net1(x)
        print(f"x0: {x0.shape}")
        x1 = torch.permute(x0, (2, 0, 1))
        print(f"x1: {x1.shape}")
        x2 = self.state_gate @ (self.state_zero + state)
        print(f"x2: {x2.shape}")
        x3 = x1 + x2
        print(f"x3: {x3.shape}")
        x4 = torch.permute(x3, (1, 2, 0))
        print(f"x4: {x4.shape}")
        x = x4
        # x = torch.permute(

...
class dyagram:
            ...
            while not queue_empty:
                try:
                    ...
                    try:
                        ...
                        tb = self.get_traceback()
                        self.log.info(f"EXCEPTION THROWN submitting dyn routing neighbors: {tb}")
    ...
    def discover_eigrp_neighbors_ssh(self, device):
        ...
        try:
            self.log.info("GETTING DEVICE TYPE")
            os = self.get_device_type(device)
            if os:
            ...
            dev.enable()
            print("AFTER SETTING ENABLE")

            if os in ['cisco_ios', 'cisco_nxos', 'cisco_xr']:

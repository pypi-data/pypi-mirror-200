...
class PrSpiders(settions):
    def __init__(self) -> None:
        ...
        logging.info('****************** @PrSpider Start  @Workers %s  @Retry %s  @Pid %s ******************'
                     % (self.workers, self.retry, self.pid))
    ...
    def Requests(cls, url=None, callback=None, headers=None, retry_time=3, method='GET', meta=None,
        ...
        if isinstance(url, str):
            url = [url]
        cls.SpiderPool(callback=callback, url=url, headers=headers, retry_time=retry_time, method=method,
                       meta=meta,
                       encoding=encoding, retry_interval=retry_interval, timeout=timeout, **kwargs)

    @classmethod
    def SpiderPool(cls, callback=None, url=None, **kwargs):
        futures = [PrSpiders.executor.submit(cls.fetch, _url, callback, **kwargs) for _url in url]

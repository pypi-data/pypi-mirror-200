...
def return_dag_ingrediants(project):
    ...
    return paths, default_args, environment, volume, volume_mount
...
def generate_airflow_dag(project: str, dag_id: str, schedule_interval, tasks: list):
    ...
    paths, default_args, environment, volume, volume_mount = return_dag_ingrediants(
        project
    )
    ...
    def return_command_args(task_dict: dict, configs: dict) -> list:
        ...
        if task_dict["task_type"] == "dbt":
            ...
            if task_dict["dbt_models"]:
                dbt_default_args_and_models = task_dict["dbt_models"] + dbt_default_args
            else:
                dbt_default_args_and_models = dbt_default_args
    ...
    for task in tasks:
        if task["upstream"] == None or task["upstream"] == "" or task["upstream"] == []:
            pass
        else:
            dependancies = []

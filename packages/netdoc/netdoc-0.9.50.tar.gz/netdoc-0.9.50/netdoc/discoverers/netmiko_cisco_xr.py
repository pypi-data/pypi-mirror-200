        ...
        def additional_tasks(task):
            ...
            for vrf in vrfs:  # pylint: disable=cell-var-from-loop
                if vrf == "default":
                    ...
                    utils.append_nornir_netmiko_task(
                        task, commands="show route connected", template="show route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show route static", template="show route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show route rip", template="show route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show route bgp", template="show route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show route eigrp", template="show route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show route ospf", template="show route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show route isis", template="show route",
                    )
                else:

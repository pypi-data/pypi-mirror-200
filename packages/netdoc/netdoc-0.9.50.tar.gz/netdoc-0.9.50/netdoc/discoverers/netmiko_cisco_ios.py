        ...
        def additional_tasks(task):
            ...
            for vrf in vrfs:  # pylint: disable=cell-var-from-loop
                if vrf == "default":
                    ...
                    utils.append_nornir_netmiko_task(
                        task, commands="show ip route connected", template="show ip route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show ip route static", template="show ip route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show ip route rip", template="show ip route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show ip route bgp", template="show ip route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show ip route eigrp", template="show ip route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show ip route ospf", template="show ip route",
                    )
                    utils.append_nornir_netmiko_task(
                        task, commands="show ip route isis", template="show ip route",
                    )
                else:

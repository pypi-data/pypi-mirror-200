...
def ingest(log):
    ...
    for item in log.parsed_output:
        ...
        if item.get("neighbor_interface") == "Routed":
            # With LL3 interfaces, use neighbor_port_id
            remote_interface_name = item.get("neighbor_port_id")
        elif item.get("neighbor_interface"):
            # Use neighbor_interface over neighbor_port_id
            remote_interface_name = item.get("neighbor_interface")
        else:
            # Last resort (neighbor_port_id can be an hex string)
            remote_interface_name = item.get("neighbor_port_id")
        ...
        if utils.parent_interface(local_interface_name) or utils.parent_interface(remote_interface_name):
            # Skip subinterfaces
            continue

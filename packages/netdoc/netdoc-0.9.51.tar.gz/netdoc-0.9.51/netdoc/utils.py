...
def test_prepare(path):
    ...
    To avoid circular dependencies, import is defined within the function.
    """
    from os import walk
    ...
    from netdoc.models import ArpTableEntry, MacAddressTableEntry, RouteTableEntry, Discoverable, Credential, DiscoveryLog
    from netdoc.schemas import credential as credential_api, discoverable as discoverable_api, discoverylog as discoverylog_api
    ...
    credential_o, created = credential_api.get_or_create(name="test-credential")
    ...
    for dirpath, dirnames, filenames in walk(path):
        for filename in filenames:
            if filename.endswith(".json"):
                ...
                with open(filepath_medata, "r") as fh:
                    # Load metadata
                    discoverablelog_json = json.load(fh)
                with open(filepath_raw_output, "r") as fh:
                    # Load raw output
                    raw_output = fh.read()

                # Create discoverable
                discoverable_o, created = discoverable_api.get_or_create(address=address, discoverable=False, credential_id=credential_o.pk, mode=discoverablelog_json.get("discoverable__mode"), site_id=site_o.pk)
                ...
                # Create discoverylog
                discoverylog_api.create(discoverable_id=discoverable_o.pk, **discoverablelog_json)

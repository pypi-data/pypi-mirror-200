...
def import_from(module, name):
    # only use for scipy interpolation
...
def plot_barrier(datafile:str='neb.h5',
    ...
    try:
        interpolate_method = getattr(__import__('scipy.interpolate', fromlist=[method]), method)
    ...
    if raw:
        ...
        pd.DataFrame({'x_interpolated':xnew, 'y_interpolated':ynew}).to_csv('raw_interpolated_xy.csv', index=False)
...
def summary(directory: str = ".", raw=False,
            **kwargs):
    """NEB任务完成总结，依次执行以下步骤：

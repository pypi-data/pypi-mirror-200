...
from hydronaut.types import Path, Decorable, Runable
...
def with_hydronaut(config_path: Path = None) -> Callable[Decorable, Runable]:
    '''
    A decorator to run custom user code with Hydronaut.
    ...
    Returns:
        A decorator for user classes and functions.
    '''

    def _with_hydronaut(user_code: Decorable) -> Runable:
        '''
        A decorator for custom user code.

        Args:
            user_code:
                Either a class that with the following methods:
                    __init__(self, config):
                        An initialization method that accepts the Hydra
                        configuration object as its sole argument.

                    __call__(self):
                        A call method that returns the experiment's objective
                        values.

                    setup(self):
                        An optional method that will be invoked before __call__ when
                        the experiment is run. This can be used to e.g. prepare
                        data.
            ...
            def get_experiment_object(self):
                ...
                # Else assume that it is a class.
                    ...
                    if not isinstance(exp_obj, Callable):
                        LOGGER.warning('%s does not create callable objects.', user_code)

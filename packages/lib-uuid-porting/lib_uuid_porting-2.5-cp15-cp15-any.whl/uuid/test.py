def unpack_iterable_mapping_call(callable, iterable=(), mapping={}) :
    """Equivalent to callable(*iterable, **mapping), but check for syntax.
"""
    ...
    for i in mapping.keys() :
        if type(i) != type("") :
            try :
                exec('raise TypeError, "keywords must be strings"')
            except SyntaxError :
                raise TypeError("keywords must be strings")
        s = s + (i + "=" + "mapping[" + repr(i) + "],")
    return eval("callable("+s+")")
...
class TestUUID(TestCase):
    ...
    def test_UUID(self):
        ...
        for (string, curly, hex, bytes, bytes_le, fields, integer, urn,
            ...
            ('12345678-1234-5678-1234-567812345678',
             ...
             0x12345678123456781234567812345678L,
    ...
    def test_exceptions(self):
        badvalue = lambda f, self=self: self.assertRaises(ValueError, f)
        badtype = lambda f, self=self: self.assertRaises(TypeError, f)
        ...
        badtype(lambda: uuid.UUID('00'*16, '\0'*16))
        badtype(lambda: uuid.UUID('00'*16, '\0'*16, '\0'*16))
        badtype(lambda: uuid.UUID('00'*16, '\0'*16, '\0'*16, (0, 0, 0, 0, 0, 0)))
        badtype(lambda: uuid.UUID('00'*16, '\0'*16, '\0'*16, (0, 0, 0, 0, 0, 0), 0))
            ...
            for bb in [[], [('bytes', b)]]:
                for bble in [[], [('bytes_le', b)]]:
                    for ii in [[], [('int', i)]]:
                        for ff in [[], [('fields', f)]]:
                            args = {}
                            for pairs in hh + bb + bble + ii + ff:
                                args[pairs[0]] = pairs[1]
                            if len(args) != 0:
                                badtype(lambda args=args: unpack_iterable_mapping_call(uuid.UUID, ('00'*16,), args))
                            if len(args) != 1:
                                badtype(lambda args=args: unpack_iterable_mapping_call(uuid.UUID, (), args))

        # Immutability.
        u = uuid.UUID(h)
        badtype(lambda u=u: setattr(u, 'hex', '00'*16))
        badtype(lambda u=u: setattr(u, 'bytes', '\0'*16))
        badtype(lambda u=u: setattr(u, 'bytes_le', '\0'*16))
        badtype(lambda u=u: setattr(u, 'fields', (0, 0, 0, 0, 0, 0)))
        badtype(lambda u=u: setattr(u, 'int', 0))
        badtype(lambda u=u: setattr(u, 'time_low', 0))
        badtype(lambda u=u: setattr(u, 'time_mid', 0))
        badtype(lambda u=u: setattr(u, 'time_hi_version', 0))
        badtype(lambda u=u: setattr(u, 'time_hi_version', 0))
        badtype(lambda u=u: setattr(u, 'clock_seq_hi_variant', 0))
        badtype(lambda u=u: setattr(u, 'clock_seq_low', 0))
        badtype(lambda u=u: setattr(u, 'node', 0))
        ...
        if TestUUID.last_node:
            if TestUUID.last_node != node:
                ...
                for s, n in TestUUID.source2node.iteritems():
                    msg = msg + "    from source %r, node was %012x\n" % (s, n)
...
"""    WARNING: uuid.getnode is unreliable on many platforms.
    ...
    def test_uuid1(self):
        ...
        for u in (uuid.uuid1(), uuid.uuid1(), uuid.uuid1(), uuid.uuid1(),\
        ...
        uuids = {}
        for u in eval('('+('uuid.uuid1(), '*999)+'uuid.uuid1())'):
            uuids[u] = 1
    ...
    def test_uuid4(self):
        ...
        for u in (uuid.uuid4(), uuid.uuid4(), uuid.uuid4(), uuid.uuid4(),\
        ...
        # Make sure the generated UUIDs are actually unique.
        uuids = []
        for u in eval('('+('uuid.uuid4(), '*999)+'uuid.uuid4())'):
            if u not in uuids:
                uuids.append(u)
...
del unpack_iterable_mapping_call

...
@click.pass_context
def init(ctx: click.Context, repo, token, db, backup, input_token):
    ...
    cfg['auth_info'] = auth
    if repo and (token or input_token):
        # init github
        cfg['repo'] = repo
        if input_token:
...
@click.argument('keywords', nargs=-1)
@click.pass_context
def save(ctx: click.Context, keywords: tuple):
    if len(keywords) == 0:
        keywords = ('',)
    db = get_db(ctx)
    for v in db.find(keywords):

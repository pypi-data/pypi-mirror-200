...
class FaciesClassifier:
            ...
            for j in range(0, c, chunk_size):
                if i + chunk_size <= df_csv.index[-1] and j + chunk_size <= c:
                    ...
                    if chunk.shape == (256, 256, 3):
                        row = {'chunk image': chunk, 'start depth': df_csv['MD'].iloc[i], 
                               'end depth': df_csv['MD'].iloc[i+chunk_size]}                       
    ...
    def predict(self, df_output):
        ...
        model_path = r'C:\Users\xjiang\AppData\Roaming\Schlumberger\Techlog\model\resnet_acc84.h5'
        ...
        # Check if predictions directory exists or not, if not then create it.
        if not os.path.exists('predictions'):
            os.makedirs('predictions')
        
        # save df predict as csv
        df_predict.to_csv(f'predictions/Prediction_Result_for_{well_name}.csv')
    ...
    def recons_image(self, df_output, df_predict, well_name):
        ...
        # save image
        img_predict = composite_image.save(f'predictions/{well_name}.png', format="PNG")
        return img_predict   
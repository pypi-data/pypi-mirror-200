...
class generatedClass(ZIDS_Pipeline):
    ...
    def build_pipeline(self):
        ...
        self.preprocessor = generatedProjectNamePreprocess(artifacts=self.artifacts, callback=self.callback)

        ## Instantiate and add predictor to the pipeline
        self.predictor = generatedProjectNamePredictor(artifacts=self.artifacts, callback=self.callback)
        self.add_component(self.predictor)

        ## Instantiate and add forcer to the pipeline
        self.forcer = generatedProjectNameForcer(artifacts=self.artifacts, callback=self.callback)
        self.add_component(self.forcer)

        ## Instantiate postprocessor - Automatically added to the pipeline
        self.postprocessor = generatedProjectNamePostprocess(artifacts=self.artifacts, callback=self.callback)

...
class OracleTools:
    ...
    @staticmethod
    def get_ddl_by_dbms(dba_conn, username, type_, obj_name):
    ...
    def get_ddl(dba_conn, username, table):
        ...
        for row in rows:
            type_ = row[3]
            name = row[2]
            ddl_sql = OracleTools.get_ddl_by_dbms(dba_conn, username, type_, name) + ';'

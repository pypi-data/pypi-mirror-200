...
class Opinum(object):
    ...
    def get_dps(self, site, source, variable, interval=None):
        try:
            data = self.get_ts(site, source, variable=variable, interval=interval)
            dps = pd.DataFrame(data)
            dps = dps.sort_values(by="date")
            dps.index = pd.to_datetime(dps["date"], format=iso)
    ...
    @token_required
    def push_ts(self, site, source, variable, dps=[]):
    ...
    @token_required
    def push_dps(self, site, source, variable, dps=None):
        try: 
            data = pd.DataFrame({ "date": dps.index.values, "value": dps.values })
            data["date"] = data["date"].dt.strftime(iso)
            data = data.to_dict(orient="records")
            return self.push_ts(site, source, variable=variable, dps=data)
        except: return
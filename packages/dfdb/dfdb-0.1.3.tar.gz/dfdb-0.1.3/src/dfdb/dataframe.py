...
class DataFrameGroupBy:
    def __init__(self, df, columns):
        ...
        self.ungrouped_columns = [col for col in self.df.columns if col not in self.columns]
    ...
    def agg(self, *args, **kwargs):
        ...
        return DataFrame(self.df.db, self.df.table, sa.select(*[self.df.table.c[col] for col in self.columns], *columns).group_by(self.columns))
        
    def transform(self, *args, **kwargs):
        raise NotImplementedError()
        
    def nunique(self):
        return DataFrame(self.df.db, self.df.table, sa.select(*self.df.table.c[tuple(self.columns)], *[sa.func.count(self.df.table.c[col].distinct()).label(col) for col in self.ungrouped_columns]).group_by(self.columns))
...
class ColumnGroupBy:
    ...
    def nunique(self):
        return Column(self.gb.df.db, self.gb.df.table, sa.select(sa.func.count(self.gb.df.table.c[self.column].distinct())).group_by(self.gb.columns))
    ...
    def agg(self, *args, **kwargs):
        ...
        if len(columns) == 1:
            return Column(self.gb.df.db, self.gb.df.table, sa.select(*columns).group_by(self.gb.columns))
        
        return DataFrame(self.gb.df.db, self.gb.df.table, sa.select(*self.gb.df.table.c[tuple(self.gb.columns)], *columns).group_by(self.gb.columns))

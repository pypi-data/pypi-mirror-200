...
from sol.models import Competitor, Match, MergedPlayer, Player, Tourney
...
def test_match(app, session, tourney_third):
    ...
    tourney_third = session.query(Tourney).get(m.idtourney)
    other_match = [m for m in tourney_third.matches if m.turn == m.board == 2][0]
    other_match.score1 = 0
    other_match.score2 = 10
    tourney_third.updateRanking()
    tourney_third.makeFinalTurn()
    session.commit()

    assert tourney_third.currentturn == 3

    result = app.post(url, (('turn', 3), ('breaker', '1')))
    assert result.json['success']
    session.expunge_all()

    q = session.query(Match).filter_by(idtourney=tourney_third.idtourney, turn=3, board=1)
    m = q.one()
    assert m.breaker == '1'
    assert m.score1 == m.score2 == 0
    assert not m.boards

    result = app.post(url, (('turn', 3), ('end_match', ''),
                            ('coins_1_1', '1'), ('queen_1', '1'),
                            ('coins_2_1', '1'), ('queen_2', '2'),
                            ('coins_3_1', '1'), ('queen_3', '1'),
                            ('coins_4_1', '1'), ('queen_4', '2'),
                            ('coins_5_1', '1'), ('queen_5', '1'),
                            ('coins_6_1', '1'), ('queen_6', '2'),
                            ('coins_7_1', '1'), ('queen_7', '1'),
                            ('coins_8_1', '1'), ('queen_8', '2'),
                            ('coins_9_1', '1'), ('queen_9', '2'),
                            ('coins_10_1', '1'), ('queen_10', '2'),
                            ('coins_11_1', '1'), ('queen_11', '2'),
                            ('coins_12_1', '1'), ('queen_12', '2'),
                            ('score1', '24'), ('score2', '0')
                            ), status=303)
    session.expunge_all()
    m = q.one()
    assert len(m.boards) == 12

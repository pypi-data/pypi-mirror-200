...
# :Copyright: © 2016, 2018–2020, 2023 Lele Gaifax
#
...
class BadgesPrintout(object):
    ...
    def drawRightSide(self, competitor, player, rank):
        ...
        c.drawString(0.2*cm, self.height-1.25*cm, gettext('Opponent'))

        # TRANSLATORS: this is the "score of this player" in the badge
        your = gettext('your')
        c.drawRightString(2.9*cm, self.height-1.25*cm, your)

        # TRANSLATORS: this is the "opponent score" in the badge
        his = gettext('his')
        c.drawRightString(3.3*cm, self.height-1.25*cm, his)
        ...
        for i, m in enumerate(pmatches):
            ...
            c.drawString(0.2*cm, h,
                         other and e(other.caption(html=False), 2.3*cm)
                         or gettext('Phantom'))
            c.drawRightString(2.9*cm, h, str(myscore))
            c.drawRightString(3.3*cm, h, str(otherscore))
        ...
        if self.tourney.championship.prizes == 'asis':
            ...
            # TRANSLATORS: this is the points in the badge
            pts = gettext('pts')
            c.drawRightString(6.7*cm, self.height-1.25*cm, pts)
            # TRANSLATORS: this is the bucholz in the badge
            bch = gettext('bch')
            ...
            for i, ctor in enumerate(self.tourney.ranking):
                ...
                c.drawRightString(6.7*cm, h, str(ctor.points))

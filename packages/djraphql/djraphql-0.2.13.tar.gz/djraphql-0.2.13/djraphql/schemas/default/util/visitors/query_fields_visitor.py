        ...
        if parent_gql_type.name in self.type_lookup:
            ...
            if field:
                ...
                for computed_field in entity_class._get_computed_fields():
                    ...
                    # If we run into a Union type as a ComputedField, just skip this
                    # because it's impossible to optimize the associated subtree.
                    if is_union_type(computed_field.graphene_type):
                        return False
...
def is_union_type(gql_type):
    terminal_type = get_terminal_gql_type(gql_type)
    return isinstance(terminal_type, type) and issubclass(terminal_type, Union)

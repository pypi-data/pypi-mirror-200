...
class AsyncEventLogger(EventLogger):
    def __init__(self, role: str = '', instance: str=''):
        ...
        self.unstruct_queue: queue.Queue[Event] = queue.Queue(maxsize=LOGGING_QUEUE_CAPACITY)
        self.struct_queue: queue.Queue[Event] = queue.Queue(maxsize=LOGGING_QUEUE_CAPACITY)
...
class _AsyncEventLoggerBackendLoop:
    def __init__(
        self,
        unstruct_queue: queue.Queue,
        unstruct_sinks: List[EventSink],
        struct_queue: queue.Queue,
        struct_sinks: List[EventSink],
    ):
        self.categories_q_original: List[queue.Queue] = [unstruct_queue, struct_queue]
        self.categories_q: List[mp.Queue[Event]] = [mp.Queue(maxsize=LOGGING_QUEUE_CAPACITY), mp.Queue(maxsize=LOGGING_QUEUE_CAPACITY)]
        ...
        self.q_conn_threads: List[threading.Thread] = []
    ...
    def start(self):
        with self.mutex:
            if self.process:
                return
            # q connector
            for q_source, q_target in zip(self.categories_q_original, self.categories_q):
                t = threading.Thread(target=self.run_q_connector, args=(q_source, q_target), daemon=True)
                self.q_conn_threads.append(t)
                t.start()
            # logging process
            self.process = mp.Process(target=self.main, daemon=True)
            self.process.start()

    def run_q_connector(self, q_source: queue.Queue, q_target: mp.Queue):
        while not self.terminated.is_set():
            evt = q_source.get()
            try:
                q_target.put_nowait(evt)
            except queue.Full:
                msg = f'{ERR_PREFIX} _AsyncEventLoggerBackendLoop shedding an event. Struct buffer grew faster than stream could be written.'
                print(msg, file=sys.stderr)

...
__intname__ = "npbackup.rustic_wrapper"
...
class RusticRunner:
    ...
    def _make_env(self) -> None:
        ...
        if self.password:
            try:
                os.environ["RUSTIC_PASSWORD"] = str(self.password)
            except TypeError:
                logger.error("Bogus rustic password")
                self.password = None
        if self.repository:
            try:
                os.environ["RUSTIC_REPOSITORY"] = str(self.repository)
            except TypeError:
                logger.error("Bogus rustic repository")
    ...
    def _remove_env(self) -> None:
        ...
        """
        os.environ["RUSTIC_PASSWORD"] = "o_O"
        os.environ["RUSTIC_REPOSITORY"] = self.repository.split(":")[0] + ":o_O"
    ...
    def executor(
        ...
        """
        Executes rustic with given command
        ...
        if exit_code == 3 and os.name == "nt" and self.ignore_cloud_files:
            ...
            # Since errors are always shown, we don't need rustic --verbose option explicitly
            cloud_error = True
    ...
    def _get_binary(self) -> None:
        """
        Make sure we find rustic binary depending on platform
        """
        ...
        if os.name == "nt":
            ...
            probe_paths = self.binary_search_paths + [
                ...
                os.path.join(os.environ.get("ProgramFiles", ""), "rustic"),
            ]
        else:
            binary = "rustic"
        ...
        for path in probe_paths:
            ...
            if os.path.isfile(probed_path):
                ...
                return
        logger.error(
            "No backup engine binary found. Please install latest binary from rustic.net"
        )
    ...
    def init(
        ...
        cmd = "init --set-version {} --set-compression {}".format(
            repository_version, compression
        )
        result, output = self.executor(cmd, errors_allowed=errors_allowed)
        if result:
            if re.search(
                r"created rustic repository ([a-z0-9]+) at .+", output, re.IGNORECASE
            ):
                return True
        else:
    ...
    def backup(
        ...
        """
        Executes rustic backup after interpreting all arguments
        """
        if not self.is_init:
            return None, None
    ...
    def raw(self, command: str) -> Tuple[bool, str]:
        """
        Execute plain rustic command without any interpretation"
        """
        if not self.is_init:
            return None

...
class Mixin(_SharedConverters):
    ...
    def get_module_source(
        ...
        grpc_return_value:InstanceInfo   = self.stub.GetModuleSource(request=moduleSourceRequest)
        ...
        for (field, value) in grpc_return_value.ListFields():
            key = field.name
            if type(value) in [VersionedModuleSource.ModuleSourceV0, VersionedModuleSource.ModuleSourceV1]:
                    result[key] = self.convertType(value)

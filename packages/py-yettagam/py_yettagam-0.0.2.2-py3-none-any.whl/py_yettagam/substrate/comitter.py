...
class SubstrateCommitter(object):
    ...
    def __upload_file(self, ip_address: str = None) -> None:
        ...
        logging.info(
            f"Uploading file {self.__file_location_path} to IP address {ip_address} ...")
        # get the file name
        file_name = os.path.basename(self.__file_location_path)
        with pysftp.Connection(host=ip_address, port=22, username="writer") as sftp:
            sftp.put(self.__file_location_path,
                     f"/ home/.metarium/inbox/{self.__chain_spec_hash}/{file_name}")
    ...
    def upload(self) -> None:
        ...
        for listener in valid_listeners:
            logging.info(
                f"Uploading Kuri {self.__kuri} to listener {listener['node']} at {listener['ip_address']} ...")
            # upload the file to the listener
            self.__upload_file(ip_address=listener["ip_address"])

        # DEBUG
        # copy the file to a local inbox folder
        test_inbox_path = f"/Users/vigneshmeenakshisundaram/Desktop/qoi/portkey/current/development/metarium/sdk/python/py-yettagam/src/.metarium/inbox/{self.__chain_spec_hash}"
        # if the inbox path does not exist, raise an error
        if not os.path.exists(test_inbox_path):
            raise FileNotFoundError(
                f"Inbox path {test_inbox_path} does not exist!")
        # copy the file to the inbox path
        shutil.copy(self.__file_location_path, test_inbox_path)

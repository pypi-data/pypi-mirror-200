...
class SubstrateCommitter(object):
    ...
    def __upload_file(self, ip_address: str = None, ssh_pub_key: str = None) -> None:
        ...
        if not isinstance(ssh_pub_key, str):
            logging.error(
                f"ssh_pub_key {ssh_pub_key} is not a string!")
            raise TypeError
        if not self._is_valid_ssh_pub_key(ssh_pub_key):
            logging.error(
                f"ssh_pub_key {ssh_pub_key} is not a valid SSH public key!")
            raise ValueError
        # get the file name
    ...
    def upload(self) -> None:
        ...
        for listener in valid_listeners:
            ...
            self.__upload_file(
                ip_address=listener["ip_address"], ssh_pub_key=listener["ssh_pub_key"])

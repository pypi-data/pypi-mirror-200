...
class SubstrateListener(object):
    ...
    def __add_writers_to_file_system(self, writers):
        ...
        with open(authorized_keys_file, "a") as f:
            for writer in writers:
                ...
                # add the writer to the authorized_keys file
    ...
    def __remove_writers_from_file_system(self, writers):
        ...
        logging.info(f"authorized_keys: {authorized_keys}")
        # remove the writers from the authorized_keys file
        with open(authorized_keys_file, "w") as f:
            for known_host in authorized_keys:
                # trim known_host
                ...
                for writer in writers:
                    if writer["ip_address"] == ip_address:
                        ...
                        break
                # if the writer is not in the writers to remove, add the writer to the authorized_keys file
                logging.info(
                    f"writer_to_remove: {writer_to_remove}")
    ...
    def __writers_from_file_system(self):
        ...
        # get authorized_keys file from reader_path/.ssh/authorized_keys
        authorized_keys_file = os.path.join(
            self.__writer_location_path, ".ssh", "authorized_keys")
        # get the ssh public keys from the authorized_keys file
        with open(authorized_keys_file, "r") as f:
            authorized_keys = f.readlines()
        # get the writers from the authorized_keys file
        for known_host in authorized_keys:
            # trim known_host
            known_host = known_host.strip()
            # ignore if known_host is empty
            ...
            ssh_pub_key = " ".join(known_host[1:])
            # append ip_address and ssh_pub_key to writers_from_file_system
            writers_from_file_system.append(
                {"ip_address": ip_address, "ssh_pub_key": ssh_pub_key})
